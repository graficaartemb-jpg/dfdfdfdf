import { Policial, PatenteCategoria, Patente, Aviso, RecrutaTeste, Movimentacao, Grupo, Treinamento, AplicacaoTreinamento, EnvioXP, ConfiguracaoSite } from '../types';
...
// Create a notice
export function criarAviso(titulo: string, conteudo: string, autor: string, tipo: 'urgente' | 'promocao' | 'geral', imageUrl?: string): Aviso {
  const avisos = obterAvisos();
  const novo: Aviso = {
    id: Date.now().toString(),
    titulo,
  ...
  enviarWebhookParaDiscord('aviso', novo);
  return novo;
}
...
export function sincronizarDadoEspecifico(tabela: string, dados: any, action: 'upsert' | 'delete' = 'upsert') {
  ...
  if (action === 'delete') {
    ...
    supabase.from(tabela).delete().eq(keyCol, dados).then(({ error }) => {
      if (error) console.error(`[Supabase Sync] Deletar em ${tabela} falhou:`, error);
    });
    return;
  }
  ...
  if (payload) {
    supabase.from(tabela).upsert(payload).then(({ error }) => {
      if (error) console.error(`[Supabase Sync] Upsert em ${tabela} falhou:`, error);
    });
  }
}
