...
import {
  ...
  Database, UploadCloud, DownloadCloud, Copy
} from 'lucide-react';
import {
...
import { Policial, PatenteCategoria, Patente, Aviso, Movimentacao, Grupo, Treinamento, AplicacaoTreinamento, EnvioXP, ConfiguracaoSite, CargoPermissao, Advertencia, Exoneracao } from '../types';
...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  const [abaAtiva, setAbaAtiva] = useState<'inicio' | 'perfil' | 'avisos' | 'promocao' | 'hierarquia' | 'grupos' | 'aulas' | 'xp' | 'gestao_site' | 'advertencias' | 'exonerar'>('inicio');
  ...
  const [gSubAba, setGSubAba] = useState<'geral' | 'permissoes' | 'supabase'>('geral');
  ...
  const lidarComCriarAviso = (e: FormEvent) => {
    ...
    criarAviso(avisoTitulo.trim(), avisoConteudo.trim(), policial.nick, avisoTipo, avisoImagemUrl.trim() || undefined);
    setAvisoTitulo('');
    setAvisoConteudo('');
    setAvisoTipo('geral');
    setAvisoImagemUrl('');
  ...
  // Universal checks
  const eMaster = policial.nick.toLowerCase() === 'diretor_almeida' || 
  ...
  const obterPermissoesUsuario = (): CargoPermissao => {
    ...
    if (eMaster) {
      return {
        abasAcessiveis: ['inicio', 'perfil', 'avisos', 'promocao', 'hierarquia', 'grupos', 'aulas', 'xp', 'gestao_site', 'advertencias', 'exonerar'],
    ...
    // Check if configured in configSite.cargosPermissoes
    const configPerm = configSite.cargosPermissoes?.[policial.cargo];
    if (configPerm) {
      return configPerm;
    }

    // Default permissions based on cargo / rank
    const eOficial = ['Diretor', 'General', 'Coronel', 'Major', 'Capitão', 'Tenente', 'Aspirante-a-Oficial', 'Primeiro Sargento', 'Segundo Sargento'].includes(policial.cargo);
    const eComando = ['Diretor', 'General', 'Coronel', 'Major'].includes(policial.cargo);
    
    return {
      abasAcessiveis: eOficial 
        ? ['inicio', 'perfil', 'avisos', 'promocao', 'hierarquia', 'grupos', 'aulas', 'xp', 'advertencias', 'exonerar'] 
        : ['inicio', 'perfil', 'avisos', 'aulas'],
  ...
  const listaCargos = patentes.length > 0 
    ...
    : ['Diretor', 'General', 'Coronel', 'Major', 'Capitão', 'Tenente', 'Aspirante-a-Oficial', 'Primeiro Sargento', 'Segundo Sargento', 'Cabo', 'Soldado', 'Recruta'];
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <aside className="lg:col-span-1 bg-zinc-950/60 border border-zinc-900 rounded-lg p-3 space-y-2">
          ...
          <nav className="flex flex-col gap-1">
            ...
            {(permissoesUser.abasAcessiveis.includes('promocao') || permissoesUser.abasAcessiveis.includes('advertencias') || permissoesUser.abasAcessiveis.includes('exonerar')) && (
                  ...
                  <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4">
                    ...
                    <form onSubmit={lidarComCriarAviso} className="space-y-4">
                      ...
                      <div>
                        <input
                          type="url"
                          placeholder="URL da Imagem / Anexo do Comunicado (opcional)..."
                      ...
                      avisos.map((aviso) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {aviso.imageUrl && (
                              <div className="mt-3 overflow-hidden rounded-lg border border-zinc-800/80 bg-zinc-950/40">
                                <img
                                  src={aviso.imageUrl}
      ...
      <li>Após a aprovação no Treinamento, o(a) instruído(a) ficará sob a responsabilidade do(a) Instrutor(a).</li>
            ...
            {abaAtiva === 'gestao_site' && tabPermitida && (
              ...
              >
                ...
                {/* Sub Tab Switcher */}
                <div className="flex bg-zinc-950/85 border border-zinc-900 rounded p-1 w-full sm:w-auto shrink-0 self-start">
                  <button
                    type="button"
