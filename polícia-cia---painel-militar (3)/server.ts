  ...
  if (!url || !key) {
    try {
      ...
      if (fs.existsSync(credsPath)) {
        ...
        if (creds.url) url = creds.url;
        if (creds.key) key = creds.key;
      }
    } catch (err) {
...
function obterSupabase() {
  ...
  try {
    ...
    console.error('[Supabase Client] Erro ao inicializar dinamicamente o cliente:', err);
    return null;
  }
}
...
async function realizarBackupESendDiscord(force = false): Promise<{ sucesso: boolean; mensagem: string }> {
  const client = obterSupabase();
  if (!client) {
    const msg = 'Supabase não configurado no servidor. Backup cancelado.';
  ...
  try {
    ...
    if (updateError) {
      console.error('[Backup] Erro ao atualizar timestamp de último backup:', updateError.message);
    }
