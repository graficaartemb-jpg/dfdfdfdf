...
import { testarConexaoSupabase, sincronizarTudoDoSupabase, exportarDadosParaSupabase, SQL_SCRIPT_GERAL, SQL_SCRIPT_RESET } from '../lib/supabase';
...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  const [gSubAba, setGSubAba] = useState<'geral' | 'permissoes' | 'supabase' | 'webhooks' | 'alterar_senhas' | 'apostilas' | 'backup'>('geral');
  ...
  const [sqlScriptOption, setSqlScriptOption] = useState<'geral' | 'reset'>('geral');
  ...
  useEffect(() => {
    ...
    if (!permissoes.abasAcessiveis.includes(abaAtiva) && abaAtiva !== 'gestao_site') {
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                    ...
                    <div className="lg:col-span-5 bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-zinc-900 pb-3 gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const script = sqlScriptOption === 'geral' ? SQL_SCRIPT_GERAL : SQL_SCRIPT_RESET;
                      ...
                      <div className="bg-zinc-950 border border-zinc-900 rounded p-3 h-48 overflow-y-auto font-mono text-[10px] text-zinc-500 leading-relaxed select-all">
                        <pre>{sqlScriptOption === 'geral' ? SQL_SCRIPT_GERAL : SQL_SCRIPT_RESET}</pre>
                      </div>
                    </div>
                  </div>
                )}
