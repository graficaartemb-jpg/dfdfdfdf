...
export function aceitarSolicitacaoCadastro(id: string, autorNick: string): { sucesso: boolean; mensagem: string } {
  ...
  // Add to active police officers list
  policiais[nickMin] = {
    nick: sol.nick,
    cargo: cargoInicial,
  ...
  const embed = {
    ...
    fields: [
      ...
      { name: '💂 Cargo Inicial', value: 'Soldado', inline: true },
    ...
    thumbnail: {
      url: `https://api.radiohabblet.com.br/imager?user=${encodeURIComponent(sol.avatarHabbo)}&gesture=std&direction=3&head_direction=3&headonly=false&size=m`
    },
    footer: {
