...
import {
  obterAvisos, criarAviso, excluirAviso, obterPoliciais, alterarPatentePolicial,
...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  const solicitarConfirmacaoApagar = (titulo: string, mensagem: string, callback: () => void) => {
    setConfirmacaoApagar({
      ...
      onConfirmar: () => {
        callback();
        setConfirmacaoApagar(prev => ({ ...prev, mostrar: false }));
      }
    });
  };
        ...
        {confirmacaoApagar.mostrar && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="w-full max-w-md bg-zinc-950 border-2 border-red-900/60 shadow-[0_0_50px_rgba(220,38,38,0.15)] rounded-lg p-6 space-y-6 relative overflow-hidden"
            >
              {/* Decorative tactical background corner slash */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-red-600/5 rotate-45 translate-x-12 -translate-y-12 border-b border-red-600/20" />
              
              <div className="flex items-start gap-4">
                <div className="p-3 bg-red-950/40 border border-red-900/50 rounded-lg shrink-0 text-red-500">
                  <ShieldAlert className="w-6 h-6" />
                </div>
                <div className="space-y-1 flex-1">
                  <h3 className="text-sm font-tactical font-extrabold text-red-500 tracking-wider uppercase">
                    {confirmacaoApagar.titulo || 'ATENÇÃO REQUERIDA'}
                  </h3>
                  <p className="text-xs text-zinc-350 font-tactical leading-relaxed whitespace-pre-line">
                    {confirmacaoApagar.mensagem}
                  </p>
                </div>
              </div>

              <div className="border-t border-zinc-900/60 pt-4 flex items-center justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setConfirmacaoApagar(prev => ({ ...prev, mostrar: false }))}
                ...
                <button
                  type="button"
                  onClick={confirmacaoApagar.onConfirmar}
                  className="px-4 py-2 bg-gradient-to-r from-red-950 to-red-800 hover:from-red-900 hover:to-red-700 text-white border border-red-900 rounded font-tactical font-semibold text-xs tracking-wider shadow-lg shadow-red-950/50 transition-all cursor-pointer uppercase"
                >
                  Confirmar Exclusão
                </button>
              </div>
