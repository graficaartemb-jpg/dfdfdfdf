...
export async function sincronizarTudoDoSupabase(): Promise<{ sucesso: boolean; mensagem: string }> {
  try {
    ...
    // Check if there are tables populated locally but empty/null in Supabase.
    // If so, we perform a push first so that we don't wipe out the user's local work!
    let needsPush = false;
    const tablesToSyncIfEmpty = [
      { key: CHAVE_CATEGORIAS, remoteData: data.categorias },
      { key: CHAVE_PATENTES, remoteData: data.patentes },
      { key: CHAVE_GRUPOS, remoteData: data.grupos },
      { key: CHAVE_AVISOS, remoteData: data.avisos },
      { key: CHAVE_TREINAMENTOS, remoteData: data.treinamentos },
      { key: CHAVE_APLICACOES_TREINAMENTOS, remoteData: data.aplicacoes_treinamentos },
      { key: CHAVE_XP_ENVIADOS, remoteData: data.xp_enviados },
      { key: CHAVE_ADVERTENCIAS, remoteData: data.advertencias },
      { key: CHAVE_EXONERACOES, remoteData: data.exoneracoes },
      { key: CHAVE_TESTES, remoteData: data.testes }
    ];

    for (const item of tablesToSyncIfEmpty) {
      const localRaw = localStorage.getItem(item.key);
      const localHasData = localRaw ? JSON.parse(localRaw).length > 0 : false;
      const remoteIsEmpty = !item.remoteData || item.remoteData.length === 0;
      if (localHasData && remoteIsEmpty) {
        console.log(`[Supabase Sync] Table ${item.key} has local data but is empty on Supabase. Triggering automatic push to preserve local work!`);
        needsPush = true;
        break;
      }
    }

    // Also check for 'policiais' table
    const localPolsRaw = localStorage.getItem(CHAVE_POLICIAIS);
    let localPolsCount = 0;
    if (localPolsRaw) {
      try {
        localPolsCount = Object.keys(JSON.parse(localPolsRaw)).length;
      } catch (e) {}
    }
    const remotePolsCount = data.policiais ? data.policiais.length : 0;
    if (localPolsCount > 2 && remotePolsCount <= 2) {
      console.log(`[Supabase Sync] Table policiais has ${localPolsCount} local members but only ${remotePolsCount} remote members. Triggering automatic push!`);
      needsPush = true;
    }
