...
import {
  ...
  obterPostsAbasCustomizadas, salvarPostAbaCustomizada, deletarPostAbaCustomizada, sincronizarDadoEspecifico
} from '../lib/db';
...
import { testarConexaoSupabase, sincronizarTudoDoSupabase, exportarDadosParaSupabase, SQL_SCRIPT_GERAL, SQL_SCRIPT_RESET, SQL_SCRIPT_MIGRAR } from '../lib/supabase';
...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  const verificarConexaoSupabase = async () => {
    try {
      ...
      if (data.configured) {
        ...
        if (data.url && !gSupabaseUrl) {
          setGSupabaseUrl(data.url);
        }
      } else {
  ...
  const handleInsertLinkPostCustom = () => {
  ...
  const handleInsertImagePostCustom = () => {
  ...
  const handleInsertYouTubePostCustom = () => {
  ...
  const handleInsertTablePostCustom = () => {
  ...
  const handleImportHTMLPostCustom = () => {
  ...
  const handleLineHeightPostCustom = () => {
  ...
  const lidarComExcluirAdvertencia = (id: string) => {
    if (!safeConfirm('Tem certeza que deseja excluir esta advertência?')) return;
  ...
  const lidarComExcluirTreinamento = (id: string) => {
    if (safeConfirm('Tem certeza que deseja excluir este treinamento? Todas as aplicações vinculadas continuarão no histórico.')) {
  ...
  const lidarComExcluirGrupo = (id: string) => {
    if (safeConfirm('Tem certeza que deseja excluir este grupo?')) {
      excluirGrupo(id);
      carregarDados();
    }
  };
  ...
  const lidarComExcluirAbaCustomizada = (id: string) => {
    ...
    try {
      ...
      localStorage.setItem('cia_posts_abas_customizadas', JSON.stringify(postsFiltrados));
  ...
  const lidarComExcluirMilitarPermanente = (nick: string) => {
    ...
    if (!safeConfirm(`ATENÇÃO CRÍTICA: Tem certeza que deseja APAGAR COMPLETAMENTE o militar @${nick} do sistema? Esta ação é irreversível e removerá todas as credenciais de acesso deste militar imediatamente.`)) {
      return;
    }
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-xs font-tactical tracking-wide transition-all cursor-pointer text-left ${
                          abaAtiva === 'setores_diversos' && customAbaAtivaId === aba.id
                            ? 'bg-gradient-to-r from-emerald-950/20 via-emerald-900/10 to-transparent border-l-2 border-emerald-600 text-zinc-100 font-semibold'
        ...
        <section className="lg:col-span-3 bg-zinc-950/30 border border-zinc-900 rounded-lg p-5 sm:p-6 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
          <AnimatePresence mode="wait">
            ...
            {abaAtiva === 'setores_diversos' && (() => {
              ...
              const podeVer = eMaster || !currentAba.permissaoVer || currentAba.permissaoVer.length === 0 || currentAba.permissaoVer.includes('todos') || currentAba.permissaoVer.includes(policial.cargo);
              if (!podeVer) {
                return (
              ...
              const podePostar = eMaster || currentAba.permissaoPostar?.includes('todos') || currentAba.permissaoPostar?.includes(policial.cargo);
                ...
                >
                  ...
                  <div className="bg-zinc-950/60 border border-emerald-900/40 rounded-lg p-5 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 relative overflow-hidden">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-base font-tactical font-bold text-emerald-500 tracking-wider flex items-center gap-2 uppercase">
                        <Sliders className="w-5 h-5" />
                        {currentAba.titulo}
                      </h3>
                      <p className="text-xs text-zinc-400 font-tactical leading-relaxed">
                        Pasta de documentos, avisos e comunicações oficiais do setor.
                      </p>
                    </div>
                    ...
                    {podePostar && (
                      <button
                        ...
                        className="px-4 py-2.5 bg-emerald-900/30 hover:bg-emerald-900/50 border border-emerald-800/50 text-emerald-300 font-tactical font-bold text-xs tracking-wider uppercase rounded transition-all cursor-pointer flex items-center gap-2"
                      >
                        <FileText className="w-4 h-4" />
                  ...
                  {podePostar && mostrarFormPostCustom && (
                    <motion.div
                      ...
                      className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4 overflow-hidden"
                    >
                      <h4 className="text-xs font-tactical font-semibold text-zinc-350 tracking-wider uppercase border-b border-zinc-900 pb-2 flex items-center gap-2">
                        <FileText className="w-4 h-4 text-emerald-500" />
                        PUBLICAR DOCUMENTO / INFORMATIVO NO SETOR
                      </h4>
                      ...
                      <form onSubmit={lidarComSalvarPostCustom} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="text-[10px] text-zinc-500 font-tactical uppercase tracking-wider block">
                              Título do Documento / Postagem
                            </label>
                            <input
                              type="text"
                              placeholder="Ex: DIRETRIZ INTERNA DE OPERAÇÕES..."
                              value={tituloNovoPostCustom}
                              onChange={(e) => setTituloNovoPostCustom(e.target.value)}
                              required
                              className="w-full bg-zinc-900/50 border border-zinc-800 text-zinc-100 font-tactical px-3 py-2.5 rounded text-xs outline-none focus:border-emerald-600"
                            />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[10px] text-zinc-500 font-tactical uppercase tracking-wider block">
                              Link de Anexo ou Imagem (Opcional)
                            </label>
                            <input
                              type="url"
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] text-zinc-500 font-tactical uppercase tracking-wider block">
                            Conteúdo do Documento / Corpo do Comunicado (Formatável)
                          </label>
                          
                          <div className="border border-zinc-900 rounded bg-zinc-900/30 overflow-hidden">
                            {/* Editor Toolbar */}
                            ...
                            {previewAtivoPostCustom ? (
                              ...
                              <div 
                                ...
                                placeholder="Escreva aqui as diretrizes, documentos ou comunicados do setor..."
                        ...
                        <div className="flex gap-2">
                          <button
                            type="submit"
                            className="px-5 py-2.5 bg-gradient-to-r from-emerald-950 via-emerald-800 to-emerald-950 text-white font-tactical font-bold text-xs tracking-wider uppercase rounded transition-all cursor-pointer shadow-lg border border-emerald-700/30"
                          >
                            Publicar no Setor
                          </button>
                          <button
                  ...
                  <div className="space-y-4">
                    {postsDestaAba.length === 0 ? (
                      <div className="p-12 text-center bg-zinc-950/40 border border-zinc-900 border-dashed rounded-lg text-zinc-500 italic text-xs">
                        Nenhum documento, portaria ou aviso publicado nesta área de setor.
                      </div>
                    ) : (
                      postsDestaAba.map((post) => {
                        const infoMembro = policiais[post.autor.toLowerCase().trim()];
                        const podeExcluir = eMaster || post.autor === policial.nick;

                        return (
                          <div key={post.id} className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4 relative">
                            {podeExcluir && (
                              <button
                            ...
                            <div className="flex items-start gap-3">
                              <div className="w-10 h-10 rounded-lg bg-zinc-900 overflow-hidden shrink-0 flex items-center justify-center border border-zinc-800">
                                <img
                              ...
                              <div className="min-w-0">
                                <h4 className="text-sm font-bold text-zinc-200 uppercase tracking-wide">{post.titulo}</h4>
                                <p className="text-[10px] text-zinc-500 font-tactical mt-0.5">
                                  Por <span className="text-emerald-500 font-semibold">@{post.autor}</span> • {post.data}
                                </p>
                              </div>
                            </div>

                            <div 
                              className="text-xs text-zinc-300 leading-relaxed font-tactical bg-zinc-900/30 p-4 border border-zinc-900 rounded-md rich-text-content prose prose-invert max-w-none"
                            ...
                            {post.fileUrl && (
                              <div className="pt-2">
                                ...
                                >
                                  ...
                                  ABRIR ARQUIVO ANEXO / LINK DE DOCUMENTO
                                </a>
                              </div>
                            )}
                          </div>
                                            ...
                                            {(() => {
                                              ...
                                              <>
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => {
                                                    const mot = prompt('Por favor, informe o motivo da reprovação desta aula:');
                                                    if (mot !== null) {
                        ...
                        <div className="bg-zinc-950/40 border border-zinc-900/80 rounded-lg p-5 space-y-4">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3">
                            {patentes.length > 0 ? (
                              ...
                              <div className="col-span-full py-6 text-center border border-dashed border-zinc-800 rounded bg-zinc-950/20">
                                <span className="text-xs text-zinc-500 font-tactical">Nenhuma patente criada ainda. Crie patentes na aba Hierarquia para definir quem pode aplicar.</span>
                              </div>
                            )}
                          </div>
                        </div>

                        {/* Seção 2: Quem pode avaliar (aprovar/reprovar) */}
                        <div className="bg-zinc-950/40 border border-zinc-900/80 rounded-lg p-5 space-y-4">
                          <div className="flex items-center gap-2">
                            <ShieldCheck className="w-4 h-4 text-emerald-500" />
                            <h4 className="text-xs font-tactical font-bold text-zinc-100 uppercase tracking-wider">Quem pode aprovar ou reprovar os resultados</h4>
                          </div>
                          <p className="text-xs text-zinc-400 font-tactical leading-relaxed">
                            Configure quais patentes podem avaliar as aulas ministradas por outros instrutores, aprovando ou reprovando os alunos para que recebam pontos e promoções.
                          </p>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3">
                            {patentes.length > 0 ? (
                              patentes.map((p) => {
                                const cargo = p.nome;
                                const isSelected = treinamentoSelecionado.permissoesAvaliadores?.includes(cargo);
                                return (
                                  <button
                                    key={p.id}
                                    type="button"
                                    onClick={() => {
                                      const list = [...(treinamentoSelecionado.permissoesAvaliadores || [])];
                                      const updated = list.includes(cargo)
                                        ? list.filter(c => c !== cargo)
                                        : [...list, cargo];
                                      const updatedTreino = { ...treinamentoSelecionado, permissoesAvaliadores: updated };
                                      setTreinamentoSelecionado(updatedTreino);
                                      salvarTreinamento(updatedTreino);
                                      carregarDados();
                                    }}
                              ...
                              <div className="col-span-full py-6 text-center border border-dashed border-zinc-800 rounded bg-zinc-950/20">
                                <span className="text-xs text-zinc-500 font-tactical">Nenhuma patente criada ainda. Crie patentes na aba Hierarquia para definir quem pode avaliar.</span>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="lg:col-span-3 bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4">
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="space-y-1">
                              <h5 className="text-xs font-bold text-emerald-400 font-tactical uppercase tracking-wider">
                                CREDENCIAIS PROTEGIDAS NO SECRETS
                              </h5>
                              <p className="text-[10px] text-zinc-350 leading-normal font-sans">
                                Como solicitado, as chaves de conexão estão configuradas com segurança na aba <strong>Secrets</strong> da plataforma. Nenhuma credencial é exibida no navegador ou gravada de forma insegura.
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="border-t border-zinc-900 pt-3 flex flex-col gap-2">
                            <div className="flex items-center justify-between text-[10px] font-mono">
                              ...
                              {statusSupabase === 'conectado' ? (
                                <span className="text-emerald-400 font-bold uppercase">ATIVO & CONECTADO</span>
                              ) : statusSupabase === 'erro' ? (
                                <span className="text-red-500 font-bold uppercase">FALHA DE CONEXÃO</span>
                              ) : (
                                <span className="text-zinc-500 font-bold uppercase">AGUARDANDO SECRETS</span>
                              )}
                            </div>
                            {gSupabaseUrl && (
                              <div className="flex items-center justify-between text-[10px] font-mono">
                                <span className="text-zinc-500 uppercase tracking-wider">Projeto URL:</span>
                                <span className="text-zinc-300">{gSupabaseUrl}</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-zinc-900/30 border border-zinc-900 rounded-lg space-y-2">
                          <h6 className="text-[10px] font-bold text-zinc-300 uppercase tracking-wider font-tactical">Como funciona a Sincronização?</h6>
                          <p className="text-[10px] text-zinc-500 leading-relaxed font-sans">
                            Quando o status estiver <strong className="text-emerald-500">CONECTADO</strong>, todos os novos policiais, avisos, logs, treinamentos, advertências e exonerações serão salvos instantaneamente e de forma persistente no seu banco Supabase remoto.
                          </p>
                          <p className="text-[10px] text-zinc-500 leading-relaxed font-sans">
                            Caso ainda não tenha as tabelas criadas no seu banco de dados, utilize o script SQL no final da página para instalá-las no seu <strong>SQL Editor</strong> do Supabase.
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 pt-2">
                            <button
                              ...
                              onClick={async () => {
                                if (!safeConfirm('Tem certeza que deseja baixar tudo do Supabase? Isso irá substituir seus dados locais temporariamente.')) return;
                            ...
                            <button
                              ...
                              disabled={statusSupabase !== 'conectado' || sincronizandoSupabase}
                              onClick={async () => {
                                if (!safeConfirm('Tem certeza que deseja exportar todos os dados locais para o Supabase? Isso pode sobrescrever dados existentes lá.')) return;
                                setSincronizandoSupabase(true);
                        ...
                        <div className="bg-red-950/15 border border-red-900/50 p-4 rounded-lg space-y-3">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const confirm1 = safeConfirm("ATENÇÃO EXTREMA: Você está prestes a apagar TODOS OS DADOS do sistema (usuários, grupos, avisos, tudo!). Esta ação é definitiva e apagará também as tabelas do Supabase se estiver conectado. Deseja continuar?");
                              if (!confirm1) return;
                              const confirm2 = safeConfirm("CONFIRMAÇÃO FINAL: Deseja realmente executar o WIPE completo de todos os dados do sistema agora?");
                              if (!confirm2) return;

                              setSincronizandoSupabase(true);
                              const res = await limparEResetarBancoDeDados();
                              if (res.sucesso) {
                                safeAlert(res.mensagem);
                                // Forçar logout e reload do site
                                localStorage.removeItem('cia_sessao_policial');
                                window.location.reload();
                              } else {
                                safeAlert("Erro ao apagar banco de dados: " + res.mensagem);
                              }
                              setSincronizandoSupabase(false);
                            }}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              safeAlert('Script SQL copiado com sucesso! Agora basta colar no SQL Editor do Supabase e clicar em Run.');
                            }}
