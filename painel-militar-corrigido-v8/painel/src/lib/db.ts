import { Policial, PatenteCategoria, Patente, Aviso, RecrutaTeste, Movimentacao, Grupo, Treinamento, AplicacaoTreinamento, EnvioXP, ConfiguracaoSite, Apostila, ApostilaParte, AbaCustomizada, PostAbaCustomizada } from '../types';
import { obterSupabaseClient } from './supabase';

// Marca o instante da última escrita local (qualquer ação: salvar, promover, excluir, etc.)
// Isso permite que o sincronismo automático periódico (pull do Supabase) SAIBA que
// acabou de acontecer uma ação local cujo envio ao Supabase pode ainda estar em
// andamento em segundo plano, e evite sobrescrever essa mudança antes dela realmente
// chegar no banco. Sem isso, uma ação (promoção, salvar apostila, etc.) podia
// "sumir"/reverter poucos segundos depois, porque o pull automático buscava o dado
// ainda desatualizado do Supabase e sobrescrevia o localStorage.
...
// janelaMs: por quanto tempo, após uma escrita local, o pull automático deve aguardar
// antes de confiar totalmente no que vem do Supabase.
export function houveEscritaLocalRecente(janelaMs: number = 20000): boolean {
  return (Date.now() - ultimaEscritaLocalEm) < janelaMs;
}

// Encrypt and decrypt helpers for security (obfuscates sensitive data in browser localStorage and in transit)
...
// Monkey-patch localStorage to transparently encrypt and decrypt CIA-specific keys in the browser
...
localStorage.getItem = function (key: string) {
  ...
  if (key.startsWith('cia_') && value.startsWith('cia_enc_')) {
    return descriptografarTexto(value.substring(8));
  }
  return value;
};

localStorage.setItem = function (key: string, value: string) {
  if (key.startsWith('cia_') && value && !value.startsWith('cia_enc_')) {
    const encrypted = 'cia_enc_' + criptografarTexto(value);
...
export function inicializarDB() {
  ...
  // Garantir que o usuário admin supremo exista (apenas como fallback inicial se não houver registros)
  const rawPols = localStorage.getItem(CHAVE_POLICIAIS);
  const policiais = rawPols ? JSON.parse(rawPols) : {};
  let alterouPols = false;
  if (!policiais['admin'] || (policiais['admin'].senhaHex !== 'admin#2026' && policiais['admin'].senhaHex !== criptografarTexto('admin#2026'))) {
    policiais['admin'] = {
      nick: 'admin',
      cargo: 'Diretor',
  ...
  if (!policiais['78k'] || (policiais['78k'].senhaHex !== '78k#2026' && policiais['78k'].senhaHex !== criptografarTexto('78k#2026'))) {
    policiais['78k'] = {
      nick: '78k',
      cargo: 'Diretor',
      dataRegistro: '2026-07-20',
...
export function obterPoliciais(): Record<string, Policial & { senhaHex: string }> {
  ...
  const policiais = raw ? JSON.parse(raw) : {};
  // A conta 'admin' nunca deve exibir XP/pontos de promoção, mesmo que algum
  // fluxo (XP, promoção, teste aprovado, sync do Supabase) tenha alterado o valor.
  if (policiais['admin']) {
    policiais['admin'].pontosPromocao = 0;
  }
  return policiais;
}

// Lista de policiais "visíveis" nas telas de usuários/pódio/ranking (esconde a conta 'admin')
...
export function registrarPolicial(
  ...
  // Enviar log de solicitação pendente para o webhook de entradas
  const embed = {
...
export function aceitarSolicitacaoCadastro(id: string, autorNick: string): { sucesso: boolean; mensagem: string } {
  ...
  // Find the 'Soldado' patente or default to 'Soldado'
  ...
  // Add to active police officers list if not already there
  if (!policiais[nickMin]) {
    policiais[nickMin] = {
      nick: sol.nick,
      cargo: cargoInicial,
...
// Reject a pending/approved registration request
export function recusarSolicitacaoCadastro(id: string, autorNick: string): { sucesso: boolean; mensagem: string } {
  ...
  // If active previously, remove
  if (policiais[nickMin]) {
    delete policiais[nickMin];
    localStorage.setItem(CHAVE_POLICIAIS, JSON.stringify(policiais));
    // Sincronizar exclusão com Supabase se necessário, mas na sincronização geral os removidos locais somem ou são sobrescritos
  }
...
export function alterarPatentePolicial(
  ...
  if (policiais[nickMin]) {
    ...
    // Resolve and set categoryId if a matching patente exists
...
export function adicionarPontos(nickPolicial: string, pontos: number): { sucesso: boolean; policial?: Policial } {
  ...
  if (policiais[nickMin]) {
    ...
    if (nickMin === 'admin') {
      policiais[nickMin].pontosPromocao = 0; // conta admin nunca acumula XP
    }
    localStorage.setItem(CHAVE_POLICIAIS, JSON.stringify(policiais));
    sincronizarPolicialComSupabase(nickPolicial);
    return { sucesso: true, policial: policiais[nickMin] };
...
export function excluirAviso(id: string): void {
  ...
  localStorage.setItem(CHAVE_AVISOS, JSON.stringify(filtrados));
...
export function salvarTeste(teste: Omit<RecrutaTeste, 'id' | 'data'>): RecrutaTeste {
  ...
  if (policiais[examMin]) {
    ...
    if (examMin === 'admin') {
      policiais[examMin].pontosPromocao = 0; // conta admin nunca acumula XP
    }
    localStorage.setItem(CHAVE_POLICIAIS, JSON.stringify(policiais));
...
export function obterCategorias(): PatenteCategoria[] {
  ...
  const list: PatenteCategoria[] = raw ? JSON.parse(raw) : [];
...
export function sincronizarDadoEspecifico(tabela: string, dados: any, action: 'upsert' | 'delete' = 'upsert') {
  ...
  try {
    ...
    if (action === 'delete') {
      ...
      supabase.from(tabela).delete().eq(keyCol, dados).then(({ error }: any) => {
        if (error) {
    ...
    // Dynamically lookup the category of the officer's cargo if not explicitly defined
    ...
    if (String(dados.nick).toLowerCase().trim() === 'admin') {
      payload.pontos_promocao = 0; // conta admin nunca acumula XP, nem no Supabase
    }
  ...
  if (payload) {
    ...
    // Uma falha aqui (rede, campo inesperado, etc.) NUNCA pode derrubar o
    // salvamento local que já ocorreu antes desta chamada. Apenas registramos o aviso.
...
export function obterPostsAbasCustomizadas(): PostAbaCustomizada[] {
  inicializarDB();
  const raw = localStorage.getItem('cia_posts_abas_customizadas');
  return raw ? JSON.parse(raw) : [];
}

export function salvarPostAbaCustomizada(post: PostAbaCustomizada): PostAbaCustomizada {
  const posts = obterPostsAbasCustomizadas();
  ...
  localStorage.setItem('cia_posts_abas_customizadas', JSON.stringify(posts));
