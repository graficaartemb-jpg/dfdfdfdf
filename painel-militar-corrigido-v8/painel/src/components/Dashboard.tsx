...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  // 15 segundos e atualiza a tela automaticamente, sem precisar recarregar.
  useEffect(() => {
    let cancelado = false;
    ...
    const intervaloSync = setInterval(async () => {
      ...
      // Se alguma ação local (promoção, salvar apostila, excluir, etc.) aconteceu
      // há pouco tempo, o envio dela ao Supabase pode ainda estar em andamento em
      // segundo plano. Pular este ciclo evita que o pull sobrescreva o localStorage
      // com o dado ainda desatualizado do banco e "reverta" a ação na tela.
  ...
  const adicionarParteAEditar = () => {
    ...
    if (parteEmEdicaoId) {
      ...
      setApPartes(prev => prev.map(p => p.id === parteEmEdicaoId
        ? { ...p, tituloParte: novaParteTitulo.trim(), conteudo: novaParteConteudo.trim() }
        : p
      ));
      setParteEmEdicaoId(null);
    } else {
