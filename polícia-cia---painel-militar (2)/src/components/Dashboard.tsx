...
import { Policial, PatenteCategoria, Patente, Aviso, Movimentacao, Grupo, CargoGrupo, Treinamento, AplicacaoTreinamento, EnvioXP, ConfiguracaoSite, CargoPermissao, Advertencia, Exoneracao, SolicitacaoCadastro, Apostila, ApostilaParte, AbaCustomizada, PostAbaCustomizada } from '../types';
...
export default function Dashboard({ policialInicial, onLogout }: DashboardProps) {
  ...
  const [modoPermissao, setModoPermissao] = useState<'cargos' | 'grupos'>('cargos');
  const [gCargoSelecionado, setGCargoSelecionado] = useState<string>('');
  const [gGrupoSelecionado, setGGrupoSelecionado] = useState<string>('');
  ...
  const [gSubAba, setGSubAba] = useState<'geral' | 'permissoes' | 'cargos_grupos' | 'supabase' | 'webhooks' | 'alterar_senhas' | 'apostilas' | 'backup' | 'abas_customizadas'>('geral');
  const [msgConfigSucesso, setMsgConfigSucesso] = useState('');
  const [msgConfigErro, setMsgConfigErro] = useState('');

  // Cargo de Grupos Management States
  const [cgGrupoId, setCgGrupoId] = useState<string>('');
  const [cgNomeCargo, setCgNomeCargo] = useState<string>('');
  const [cgNickPessoa, setCgNickPessoa] = useState<Record<string, string>>({});
  ...
  const [treinoOrdem, setTreinoOrdem] = useState<number>(1);
  ...
  const lidarComSalvarPerfil = async (e: FormEvent) => {
    ...
    try {
      const atualizado = atualizarPerfilPolicial(policial.nick, editBio, policial.nick, policial.fotoHabblet);
      setPolicial(atualizado);
      setMsgPerfilSucesso('Biografia militar atualizada com sucesso!');
      carregarDados();
  ...
  const lidarComSalvarTreinamento = (e: FormEvent) => {
    ...
    const novoTreino: Treinamento = {
      ...
      ordem: typeof treinoOrdem === 'number' && treinoOrdem > 0 ? treinoOrdem : 1
    };
  ...
  const abrirCriarTreinamento = () => {
    ...
    setTreinoPermissoes([]);
    setTreinoCargosQuePodemVer(['todos']);
    setTreinoPermissoesAvaliadores(['todos']);
    setTreinoOrdem(treinamentos.length > 0 ? Math.max(...treinamentos.map(t => t.ordem ?? 0)) + 1 : 1);
  ...
  // Reorder Training by position number directly
  const alterarOrdemTreinamentoNumero = (t: Treinamento, novaOrdem: number) => {
    if (isNaN(novaOrdem) || novaOrdem < 1) return;
    const list = [...treinamentos].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
    list.forEach((item, idx) => {
      if (typeof item.ordem !== 'number') {
        item.ordem = idx + 1;
      }
    ...
    const targetIdx = list.findIndex(item => item.id === t.id);
    if (targetIdx === -1) return;

    const [removed] = list.splice(targetIdx, 1);
    const destIndex = Math.max(0, Math.min(novaOrdem - 1, list.length));
  ...
  // Reorder Training by arrows
  const reordenarTreinamento = (t: Treinamento, direcao: 'up' | 'down') => {
    const list = [...treinamentos].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
    list.forEach((item, idx) => {
      if (typeof item.ordem !== 'number') {
        item.ordem = idx + 1;
    ...
    if (direcao === 'up' && idx > 0) {
      ...
      prev.ordem = list[idx].ordem;
      list[idx].ordem = tempOrdem;
      salvarTreinamento(prev);
      salvarTreinamento(list[idx]);
    } else if (direcao === 'down' && idx < list.length - 1) {
      const next = list[idx + 1];
      const tempOrdem = next.ordem;
      next.ordem = list[idx].ordem;
      list[idx].ordem = tempOrdem;
      salvarTreinamento(next);
      salvarTreinamento(list[idx]);
    }
    carregarDados();
  };
  ...
  const lidarComSalvarGrupo = (e: FormEvent) => {
    ...
    const novoGrupo: Grupo = {
      ...
      cargos: grupoEditando ? grupoEditando.cargos : []
    };
  ...
  // Cargo de Grupos Handlers
  const lidarComCriarCargoGrupo = (e: FormEvent) => {
    e.preventDefault();
    if (!cgGrupoId) {
      setMsgConfigErro('Selecione um grupo para criar o cargo.');
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }
    if (!cgNomeCargo.trim()) {
      setMsgConfigErro('Informe o nome do cargo para o grupo.');
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }

    const grp = grupos.find(g => g.id === cgGrupoId);
    if (!grp) {
      setMsgConfigErro('Grupo não encontrado.');
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }

    const nomeLimpo = cgNomeCargo.trim();
    const cargosAtuais = grp.cargos || [];

    if (cargosAtuais.some(c => c.nome.toLowerCase() === nomeLimpo.toLowerCase())) {
      setMsgConfigErro(`O grupo "${grp.nome}" já possui um cargo com o nome "${nomeLimpo}".`);
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }

    const novoCargo: CargoGrupo = {
    ...
    const grupoAtualizado: Grupo = {
      ...grp,
      cargos: [...cargosAtuais, novoCargo]
    };

    salvarGrupo(grupoAtualizado);
    setGrupos(obterGrupos());
    setCgNomeCargo('');
    setMsgConfigSucesso(`Cargo "${nomeLimpo}" criado com sucesso para o grupo ${grp.nome}!`);
    setTimeout(() => setMsgConfigSucesso(''), 4000);
  };

  const lidarComExcluirCargoGrupo = (grupoId: string, cargoId: string, nomeCargo: string) => {
    if (!safeConfirm(`Tem certeza de que deseja excluir o cargo "${nomeCargo}" deste grupo?`)) return;
    ...
    const grupoAtualizado: Grupo = {
      ...grp,
      cargos: (grp.cargos || []).filter(c => c.id !== cargoId)
    };

    salvarGrupo(grupoAtualizado);
    setGrupos(obterGrupos());
    setMsgConfigSucesso(`Cargo "${nomeCargo}" excluído com sucesso!`);
    setTimeout(() => setMsgConfigSucesso(''), 4000);
  };

  const lidarComAdicionarPessoaCargoGrupo = (grupoId: string, cargoId: string) => {
    const nickInput = (cgNickPessoa[cargoId] || '').trim();
    if (!nickInput) {
      setMsgConfigErro('Digite o nick do policial para adicionar ao cargo.');
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }
    ...
    const cargosAtuais = grp.cargos || [];
    const cargoIndex = cargosAtuais.findIndex(c => c.id === cargoId);
    if (cargoIndex === -1) return;

    const cargoAlvo = cargosAtuais[cargoIndex];
    const nickNormalizado = nickInput.trim();

    if ((cargoAlvo.membros || []).some(m => m.toLowerCase().trim() === nickNormalizado.toLowerCase())) {
      setMsgConfigErro(`O policial "${nickNormalizado}" já possui o cargo "${cargoAlvo.nome}".`);
      setTimeout(() => setMsgConfigErro(''), 4000);
      return;
    }

    const novosMembrosCargo = [...(cargoAlvo.membros || []), nickNormalizado];
    const novosCargos = [...cargosAtuais];
    novosCargos[cargoIndex] = {
      ...cargoAlvo,
      membros: novosMembrosCargo
    };

    const membrosGrupoAtuais = Array.isArray(grp.membros) ? grp.membros : [];
    const jaMembroGrupo = membrosGrupoAtuais.some(m => m.toLowerCase().trim() === nickNormalizado.toLowerCase());
    const novosMembrosGrupo = jaMembroGrupo ? membrosGrupoAtuais : [...membrosGrupoAtuais, nickNormalizado];

    const grupoAtualizado: Grupo = {
      ...grp,
      membros: novosMembrosGrupo,
      cargos: novosCargos
    };

    salvarGrupo(grupoAtualizado);
    setGrupos(obterGrupos());
    setCgNickPessoa(prev => ({ ...prev, [cargoId]: '' }));
    setMsgConfigSucesso(`Policial "${nickNormalizado}" adicionado ao cargo "${cargoAlvo.nome}"!`);
    setTimeout(() => setMsgConfigSucesso(''), 4000);
  };

  const lidarComRemoverPessoaCargoGrupo = (grupoId: string, cargoId: string, nickRemover: string) => {
    const grp = grupos.find(g => g.id === grupoId);
    if (!grp) return;
    ...
    const cargoAlvo = cargosAtuais[cargoIndex];
    const nickNormalizado = nickRemover.toLowerCase().trim();

    const novosMembrosCargo = (cargoAlvo.membros || []).filter(m => m.toLowerCase().trim() !== nickNormalizado);
    const novosCargos = [...cargosAtuais];
    novosCargos[cargoIndex] = {
      ...cargoAlvo,
      membros: novosMembrosCargo
    ...
    salvarGrupo(grupoAtualizado);
    setGrupos(obterGrupos());
    setMsgConfigSucesso(`Policial "${nickRemover}" removido do cargo "${cargoAlvo.nome}".`);
    setTimeout(() => setMsgConfigSucesso(''), 4000);
  };
  ...
  const lidarComExcluirMilitarPermanente = (nick: string) => {
    if (nick.toLowerCase() === 'policecia' || nick.toLowerCase() === 'admin') {
      safeAlert('Não é permitido excluir o usuário Administrador Supremo (PoliceCIA).');
      return;
    }
  ...
  const carregarPermissoesGrupo = (grupoId: string) => {
    setGGrupoSelecionado(grupoId);
    if (!grupoId) return;

    const permExistente = configSite.gruposPermissoes?.[grupoId];
    if (permExistente) {
      setTempAbas(permExistente.abasAcessiveis || []);
      setTempPromover(!!permExistente.podePromoverRebaixar);
      setTempAvisos(!!permExistente.podePostarAvisos);
      ...
      setTempAbas([]);
      setTempPromover(false);
      setTempAvisos(false);
      setTempXP(false);
      setTempTreinamentos(false);
      setTempAvaliar(false);
      setTempGrupos(false);
      setTempHierarquia(false);
      setTempExcluirLogsTreinamento(false);
      setTempReavaliarTreinamento(false);
      setTempExcluirAvisos(false);
      setTempExcluirPoliciais(false);
      setTempExcluirApostilas(false);
    }
  };
  ...
  const lidarComSalvarPermissoesGrupo = (e: FormEvent) => {
    e.preventDefault();
    if (!gGrupoSelecionado) {
      setMsgConfigErro('Selecione um grupo para configurar.');
      return;
    }

    const grpObj = grupos.find(g => g.id === gGrupoSelecionado);
    const grpNome = grpObj ? grpObj.nome : gGrupoSelecionado;

    if (!safeConfirm(`Tem certeza de que deseja modificar as permissões para o grupo ${grpNome}?`)) return;

    try {
      const novosGruposPermissoes = {
        ...(configSite.gruposPermissoes || {}),
        [gGrupoSelecionado]: {
          abasAcessiveis: tempAbas,
          podePromoverRebaixar: tempPromover,
          podePostarAvisos: tempAvisos,
      ...
      const novaConfig = {
        ...configSite,
        gruposPermissoes: novosGruposPermissoes
      };

      salvarConfiguracaoSite(novaConfig);
      setConfigSite(novaConfig);
      setMsgConfigSucesso(`Permissões do grupo "${grpNome}" atualizadas com sucesso!`);
      carregarDados();
      setTimeout(() => setMsgConfigSucesso(''), 4000);
    } catch (err) {
      console.error(err);
      setMsgConfigErro('Ocorreu um erro ao salvar as permissões do grupo.');
      setTimeout(() => setMsgConfigErro(''), 4000);
    }
  };

  const DEFAULT_BADGE = 'https://www.habblet.com.br/habbo-imaging/badge/b26134s36244s44104s41014s06014801bbd65ce6922.gif';
  const SHIELD_SVG = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="%23dc2626" stroke="%23f87171" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>`;

  const obterBadgeUrl = (urlOrCode: string) => {
    if (!urlOrCode || typeof urlOrCode !== 'string' || urlOrCode.trim() === '') {
      return DEFAULT_BADGE;
    }
    let limpo = urlOrCode.trim();

    // Extrair URL caso colada como tag HTML <img> ou BBCode [img]
    if (limpo.includes('<img') && limpo.includes('src=')) {
      const match = limpo.match(/src=["']([^"']+)["']/i);
      if (match) limpo = match[1].trim();
    } else if (limpo.includes('[img]') && limpo.includes('[/img]')) {
      const match = limpo.match(/\[img\](.*?)\[\/img\]/i);
      if (match) limpo = match[1].trim();
    }

    // Se já começa com http://, https:// ou data:
    if (limpo.startsWith('http://') || limpo.startsWith('https://') || limpo.startsWith('data:image/')) {
      return limpo;
    }

    // Se for um link web (Discord, Imgur, ibb.co, com barra /, com extensão de imagem ou cdn/media)
    const eLinkWeb = limpo.includes('discord') ||
                     limpo.includes('imgur') ||
                     limpo.includes('ibb.co') ||
                     limpo.includes('/') ||
                     /\.(png|jpg|jpeg|gif|webp|svg)(\?.*)?$/i.test(limpo) ||
                     limpo.startsWith('cdn.') ||
                     limpo.startsWith('media.');

    if (eLinkWeb) {
      return limpo.startsWith('//') ? `https:${limpo}` : `https://${limpo}`;
    }

    // Caso contrário, trata como Código de Emblema Habbo
    let code = limpo.replace(/^\/+|\/+$/g, '');
    if (code.toLowerCase().endsWith('.gif')) {
      code = code.slice(0, -4);
    }
    return `https://www.habblet.com.br/habbo-imaging/badge/${code}.gif`;
  };

  const handleBadgeError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    ...
    if (step === '0') {
      ...
      if (currentSrc.includes('cdn.discordapp.com')) {
        target.src = currentSrc.replace('cdn.discordapp.com', 'media.discordapp.net');
      } else if (currentSrc.includes('media.discordapp.net')) {
        target.src = currentSrc.replace('media.discordapp.net', 'cdn.discordapp.com');
      } else if (currentSrc.includes('habblet.com.br/habbo-imaging/badge/')) {
        target.src = currentSrc.replace('www.habblet.com.br', 'images.habbo.com');
      } else if (currentSrc.includes('images.habbo.com/habbo-imaging/badge/')) {
        target.src = currentSrc.replace('images.habbo.com', 'www.habblet.com.br');
      } else {
        target.src = DEFAULT_BADGE;
      }
    } else if (step === '1') {
      target.dataset.fallbackStep = '2';
      if (!currentSrc.includes('habbo-imaging/badge/')) {
        target.src = DEFAULT_BADGE;
      } else {
  ...
  const eMaster = policial.nick.toLowerCase() === 'admin' ||
                  policial.nick.toLowerCase() === 'policecia' ||
  ...
  // Get permissions for current user's cargo and groups
  const obterPermissoesUsuario = (): CargoPermissao => {
    ...
    const permExistenteCargo = configSite.cargosPermissoes?.[userCargo];
    ...
    let abas: string[] = [];
    let podePromoverRebaixar = false;
    let podePostarAvisos = false;
    let podeEnviarXP = false;
    let podeCriarTreinamentos = false;
    let podeAvaliarAulas = false;
    let podeGerenciarGrupos = false;
    let podeGerenciarHierarquia = false;
    let podeExcluirLogsTreinamento = false;
    let podeReavaliarTreinamento = false;
    let podeExcluirAvisos = false;
    let podeExcluirPoliciais = false;
    let podeExcluirApostilas = false;

    if (permExistenteCargo) {
      abas = [...(permExistenteCargo.abasAcessiveis || ['inicio', 'perfil', 'avisos', 'aulas'])];
      if (eComando && !abas.includes('aceitar_membros')) {
        abas.push('aceitar_membros');
      }
      podePromoverRebaixar = !!permExistenteCargo.podePromoverRebaixar;
      podePostarAvisos = !!permExistenteCargo.podePostarAvisos;
      podeEnviarXP = !!permExistenteCargo.podeEnviarXP;
      podeCriarTreinamentos = !!permExistenteCargo.podeCriarTreinamentos;
      podeAvaliarAulas = !!permExistenteCargo.podeAvaliarAulas;
      podeGerenciarGrupos = !!permExistenteCargo.podeGerenciarGrupos;
      podeGerenciarHierarquia = !!permExistenteCargo.podeGerenciarHierarquia;
      podeExcluirLogsTreinamento = !!permExistenteCargo.podeExcluirLogsTreinamento;
      podeReavaliarTreinamento = !!permExistenteCargo.podeReavaliarTreinamento;
      podeExcluirAvisos = !!permExistenteCargo.podeExcluirAvisos;
      podeExcluirPoliciais = !!permExistenteCargo.podeExcluirPoliciais;
      podeExcluirApostilas = !!permExistenteCargo.podeExcluirApostilas;
    } else {
      abas = eComando
    ...
    // Merge permissions from user's groups
    const userNickClean = policial.nick.toLowerCase().trim();
    const meusGrupos = (grupos || []).filter(g => Array.isArray(g.membros) && g.membros.map(m => m.toLowerCase().trim()).includes(userNickClean));

    if (configSite.gruposPermissoes) {
      meusGrupos.forEach(g => {
        const permGrupo = configSite.gruposPermissoes?.[g.id];
        if (permGrupo) {
          if (Array.isArray(permGrupo.abasAcessiveis)) {
            permGrupo.abasAcessiveis.forEach(aba => {
              if (!abas.includes(aba)) {
                abas.push(aba);
              }
            });
          }
          podePromoverRebaixar = podePromoverRebaixar || !!permGrupo.podePromoverRebaixar;
          podePostarAvisos = podePostarAvisos || !!permGrupo.podePostarAvisos;
          podeEnviarXP = podeEnviarXP || !!permGrupo.podeEnviarXP;
          podeCriarTreinamentos = podeCriarTreinamentos || !!permGrupo.podeCriarTreinamentos;
          podeAvaliarAulas = podeAvaliarAulas || !!permGrupo.podeAvaliarAulas;
          podeGerenciarGrupos = podeGerenciarGrupos || !!permGrupo.podeGerenciarGrupos;
          podeGerenciarHierarquia = podeGerenciarHierarquia || !!permGrupo.podeGerenciarHierarquia;
          podeExcluirLogsTreinamento = podeExcluirLogsTreinamento || !!permGrupo.podeExcluirLogsTreinamento;
          podeReavaliarTreinamento = podeReavaliarTreinamento || !!permGrupo.podeReavaliarTreinamento;
          podeExcluirAvisos = podeExcluirAvisos || !!permGrupo.podeExcluirAvisos;
          podeExcluirPoliciais = podeExcluirPoliciais || !!permGrupo.podeExcluirPoliciais;
          podeExcluirApostilas = podeExcluirApostilas || !!permGrupo.podeExcluirApostilas;
        }
      });
    }
    ...
    return {
      abasAcessiveis: abas,
      podePromoverRebaixar,
      podePostarAvisos,
      podeEnviarXP,
      podeCriarTreinamentos,
      podeAvaliarAulas,
      podeGerenciarGrupos,
      podeGerenciarHierarquia,
      podeExcluirLogsTreinamento,
      podeReavaliarTreinamento,
      podeExcluirAvisos,
      podeExcluirPoliciais,
      podeExcluirApostilas,
    };
  };

  const permissoesUser = obterPermissoesUsuario();

  // Check if current active tab is accessible. Restrict "gestao_site" strictly to Master/Admin users.
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <aside className="lg:col-span-1 bg-zinc-950/60 border border-zinc-900 rounded-lg p-3 space-y-2">
          ...
          <nav className="flex flex-col gap-1">
                ...
                {setoresDiversosAberto && (
                  ...
                  >
                    {(configSite.abasCustomizadas || []).filter(aba => {
                      ...
                      const temGrupos = aba.gruposPermitidosVer && aba.gruposPermitidosVer.length > 0;
                      const pertenceGrupo = temGrupos ? aba.gruposPermitidosVer!.some(gid => userGroupIds.includes(gid)) : true;
                      if (temGrupos && !pertenceGrupo) return false;

                      if (!aba.permissaoVer || aba.permissaoVer.length === 0 || aba.permissaoVer.includes('todos')) return true;
                      return aba.permissaoVer.includes(policial.cargo);
                    }).map((aba) => (
                      <button
                        key={aba.id}
                        onClick={() => {
            ...
            {(permissoesUser.abasAcessiveis.includes('promocao') || permissoesUser.abasAcessiveis.includes('advertencias') || permissoesUser.abasAcessiveis.includes('exonerar') || permissoesUser.abasAcessiveis.includes('aceitar_membros') || permissoesUser.abasAcessiveis.includes('apostilas')) && (
              <div className="border-t border-zinc-900/60 pt-2 mt-1">
                <button
        ...
        <section className="lg:col-span-3 bg-zinc-950/30 border border-zinc-900 rounded-lg p-5 sm:p-6 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
          <AnimatePresence mode="wait">
                        ...
                        <div className="space-y-3.5 md:col-span-2">
                          ...
                          <div className="pt-2.5 border-t border-zinc-900">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {grupos.filter(g => Array.isArray(g.membros) && g.membros.some(m => m.toLowerCase().trim() === militarSelecionado.nick.toLowerCase().trim())).length > 0 ? (
                                grupos.filter(g => Array.isArray(g.membros) && g.membros.some(m => m.toLowerCase().trim() === militarSelecionado.nick.toLowerCase().trim())).map(g => {
                                  const badgeUrl = obterBadgeUrl(g.urlImagem);
                                  const cargosDoMilitar = (g.cargos || []).filter(c => Array.isArray(c.membros) && c.membros.some(m => m.toLowerCase().trim() === militarSelecionado.nick.toLowerCase().trim()));
                                  return (
                                    <div key={g.id} className="flex items-center gap-2 px-2.5 py-1.5 bg-zinc-900 border border-zinc-800 rounded-lg" title={`${g.nome} [${g.sigla.toUpperCase()}]`}>
                                      ...
                                      <div className="flex flex-col">
                                        <div className="flex items-center gap-1">
                                          <span className="text-[10px] text-zinc-200 font-bold font-tactical">{g.nome}</span>
                                          <span className="text-[9px] text-zinc-400 font-mono">[{g.sigla.toUpperCase()}]</span>
                                        </div>
                                        {cargosDoMilitar.length > 0 && (
                                          <div className="flex flex-wrap gap-1 mt-0.5">
                                            {cargosDoMilitar.map(c => (
                                              <span key={c.id} className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-tactical font-bold uppercase bg-amber-950/70 border border-amber-600/50 text-amber-300 rounded shadow-sm">
              ...
              >
                ...
                <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {grupos.filter(g => Array.isArray(g.membros) && g.membros.some(m => m.toLowerCase().trim() === policial.nick.toLowerCase().trim())).length > 0 ? (
                      grupos.filter(g => Array.isArray(g.membros) && g.membros.some(m => m.toLowerCase().trim() === policial.nick.toLowerCase().trim())).map(g => {
                        const badgeUrl = obterBadgeUrl(g.urlImagem);
                        const cargosDoMilitar = (g.cargos || []).filter(c => Array.isArray(c.membros) && c.membros.some(m => m.toLowerCase().trim() === policial.nick.toLowerCase().trim()));
                        return (
                          <div 
                            key={g.id} 
                            className="flex items-center gap-2.5 px-3.5 py-2.5 bg-zinc-900/50 hover:bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-lg transition-all cursor-pointer"
                            onClick={() => {
                              setAbaAtiva('grupos');
                              setAbaGrupos('meus');
                            }}
                          ...
                          >
                            <div className="w-7 h-7 bg-zinc-950 rounded overflow-hidden flex items-center justify-center p-0.5 shrink-0 border border-zinc-800">
                              <img
                                src={badgeUrl}
                                alt={g.nome}
                            ...
                            <div className="text-left space-y-0.5">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <span className="text-xs text-zinc-200 font-bold font-tactical">{g.nome}</span>
                ...
                <form onSubmit={lidarComSalvarPerfil} className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    >
                      ...
                      <span>{salvandoPerfil ? 'SALVANDO...' : 'SALVAR BIOGRAFIA'}</span>
                    </button>
                  </div>
                </form>
              </motion.div>
            ...
            {abaAtiva === 'setores_diversos' && (() => {
              ...
              const userGroupIds = grupos.filter(g => g.membros.includes(policial.nick)).map(g => g.id);
              const temGruposVer = currentAba.gruposPermitidosVer && currentAba.gruposPermitidosVer.length > 0;
              const pertenceGrupoVer = temGruposVer ? currentAba.gruposPermitidosVer!.some(gid => userGroupIds.includes(gid)) : true;
              const cargoPermitidoVer = !currentAba.permissaoVer || currentAba.permissaoVer.length === 0 || currentAba.permissaoVer.includes('todos') || currentAba.permissaoVer.includes(policial.cargo);

              const podeVer = eMaster || (pertenceGrupoVer && cargoPermitidoVer);
              if (!podeVer) {
                return (
                  <div className="p-8 text-center bg-zinc-950/60 border border-red-900/40 rounded-lg text-red-400 font-tactical uppercase tracking-wider">
                    🔒 Acesso Restrito: Suas credenciais e grupos atuais não permitem visualizar esta área.
                  </div>
                );
              }

              const temGruposPostar = currentAba.gruposPermitidosPostar && currentAba.gruposPermitidosPostar.length > 0;
              const pertenceGrupoPostar = temGruposPostar ? currentAba.gruposPermitidosPostar!.some(gid => userGroupIds.includes(gid)) : true;
              const cargoPermitidoPostar = currentAba.permissaoPostar?.includes('todos') || currentAba.permissaoPostar?.includes(policial.cargo);

              const podePostar = eMaster || (pertenceGrupoPostar && cargoPermitidoPostar);
                  ...
                  <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 sm:p-6 space-y-6">
                    ...
                    <form onSubmit={lidarComSalvarTreinamento} className="space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-1.5">
                          <label className="block text-[11px] text-zinc-400 font-tactical uppercase tracking-wider font-semibold">Ordem (#)</label>
                          <input
                            type="number"
                            min={1}
                            value={treinoOrdem}
                            onChange={(e) => setTreinoOrdem(parseInt(e.target.value, 10) || 1)}
                            className="w-full bg-zinc-950 border border-zinc-850 text-xs text-amber-500 font-bold px-3.5 py-2.5 rounded focus:border-red-600 outline-none font-mono"
                          />
                        </div>
                      </div>
                        ...
                        {!mostrarFormAplicacao ? (
                          <div className="space-y-4">
                            <div className="flex justify-between items-center">
                              ...
                              {(() => {
                                const isMasterUser = ['admin', 'diretor_almeida', 'l.petrus', 'vokhan'].includes(policial.nick.toLowerCase()) || policial.cargo?.toLowerCase() === 'diretor';
                                const canApplyTreinamento = isMasterUser || (
                                  treinamentoSelecionado.permissoesCargos && treinamentoSelecionado.permissoesCargos.length > 0
                                    ? (treinamentoSelecionado.permissoesCargos.includes('todos') || treinamentoSelecionado.permissoesCargos.includes(policial.cargo))
                                              ...
                                              {(() => {
                                                 ...
                                                 const canEvaluateApp = isMasterUser || (
                                                   treinamentoSelecionado.permissoesAvaliadores && treinamentoSelecionado.permissoesAvaliadores.length > 0
                                                     ? (treinamentoSelecionado.permissoesAvaliadores.includes('todos') || treinamentoSelecionado.permissoesAvaliadores.includes(policial.cargo))
                  ...
                  <div className="space-y-6">
                    ...
                    {filtroAbaAulas === 'aulas' ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {treinamentos
                          .filter((t) => {
                            const isMasterUser = ['admin', 'diretor_almeida', 'l.petrus', 'vokhan'].includes(policial.nick.toLowerCase()) || policial.cargo?.toLowerCase() === 'diretor' || permissoesUser.podeCriarTreinamentos;
                            if (isMasterUser) return true;
                            if (!t.cargosQuePodemVer || t.cargosQuePodemVer.length === 0 || t.cargosQuePodemVer.includes('todos')) return true;
                            return t.cargosQuePodemVer.includes(policial.cargo);
                          })
                          .map((t, index) => {
                            ...
                            <div key={t.id} className="bg-zinc-950/40 border border-zinc-900/80 rounded-lg p-5 flex flex-col justify-between hover:border-zinc-800 transition-all gap-4">
                              <div className="space-y-2">
                                <div className="flex items-start justify-between gap-2">
                                  <div className="flex items-center gap-2 min-w-0">
                                    {permissoesUser.podeCriarTreinamentos ? (
                                      <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-0.5 text-[10px] font-mono text-zinc-400 shrink-0" title="Alterar ordem / posição do treinamento">
                                        <span className="text-zinc-500 font-tactical uppercase text-[9px]">Ord:</span>
                                        <select
                                          value={index + 1}
                                          onChange={(e) => alterarOrdemTreinamentoNumero(t, parseInt(e.target.value, 10))}
                                          className="bg-zinc-950 text-amber-500 font-bold border border-zinc-800 rounded px-1.5 py-0.5 text-[10px] outline-none cursor-pointer"
                                        >
                                          {treinamentos.map((_, i) => (
                                      ...
                                      <span className="text-[9px] font-mono text-zinc-500 bg-zinc-900/80 px-1.5 py-0.5 rounded border border-zinc-850 shrink-0">
                                        #{index + 1}
                                      </span>
                                    )}
                                    <h4 className="text-xs font-tactical font-bold text-zinc-100 leading-snug tracking-wider hover:text-red-500 transition-all cursor-pointer truncate" onClick={() => { setTreinamentoSelecionado(t); setSubAbaAtiva('conteudo'); }}>
                                      {t.titulo}
                                    </h4>
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-900/60 pt-3">
                                ...
                                <div className="flex items-center gap-2 shrink-0 ml-auto">
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-850 hover:border-zinc-700 text-zinc-300 hover:text-zinc-100 text-[10px] font-tactical font-semibold rounded cursor-pointer transition-all flex items-center gap-1.5"
                                  ...
                                  {permissoesUser.podeCriarTreinamentos && (
                                    <>
                                      ...
                                      <button
                                        ...
                                        className="p-1.5 bg-red-950/20 hover:bg-red-900/40 border border-red-900/40 hover:border-red-600 text-red-400 hover:text-red-200 rounded cursor-pointer transition-all"
            ...
            {abaAtiva === 'gestao_site' && tabPermitida && (
              ...
              >
                ...
                <div className="flex flex-wrap bg-zinc-950/85 border border-zinc-900 rounded p-1 w-full sm:w-auto shrink-0 self-start gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setGSubAba('cargos_grupos')}
                    className={`px-5 py-2 rounded text-xs font-tactical font-semibold tracking-wider transition-all cursor-pointer uppercase ${
                      gSubAba === 'cargos_grupos'
                        ? 'bg-amber-950/50 text-amber-400 border border-amber-900/30 font-bold'
                        : 'text-zinc-500 hover:text-zinc-300'
                    }`}
                  >
                ...
                {gSubAba === 'permissoes' && (
                  <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-6">
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between border-b border-zinc-900 pb-4">
                      <div className="space-y-1">
                        <h4 className="text-sm font-tactical font-bold text-zinc-300 tracking-wider uppercase flex items-center gap-2">
                          <ShieldCheck className="w-4 h-4 text-amber-500" />
                          Matriz Geral de Permissões (RBAC / ABAC)
                        </h4>
                        <p className="text-xs text-zinc-500 font-tactical">
                          Configure acessos a abas e ações operacionais por <strong className="text-zinc-300">Cargo Militar</strong> ou por <strong className="text-amber-500">Grupos / Departamentos</strong>.
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 bg-zinc-900/80 p-1 rounded-lg border border-zinc-800">
                        <button
                          type="button"
                          onClick={() => {
                            setModoPermissao('cargos');
                            if (gCargoSelecionado) carregarPermissoesCargo(gCargoSelecionado);
                          }}
                          className={`px-3 py-1.5 rounded text-xs font-tactical font-bold uppercase tracking-wider cursor-pointer transition-all ${
                            modoPermissao === 'cargos'
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setModoPermissao('grupos');
                            if (gGrupoSelecionado) carregarPermissoesGrupo(gGrupoSelecionado);
                          }}
                          className={`px-3 py-1.5 rounded text-xs font-tactical font-bold uppercase tracking-wider cursor-pointer transition-all ${
                            modoPermissao === 'grupos'
                              ? 'bg-amber-600 text-white shadow-md'
                              : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'
                          }`}
                        >
                    ...
                    <div className="bg-zinc-900/30 p-4 border border-zinc-900 rounded-lg flex flex-col md:flex-row gap-4 items-center justify-between">
                      <div className="space-y-0.5">
                        <span className="text-xs font-tactical font-bold text-amber-500 uppercase tracking-wider block">
                          {modoPermissao === 'cargos' ? 'Selecione o Cargo Militar' : 'Selecione o Grupo / Departamento'}
                        </span>
                        <p className="text-[10px] text-zinc-500">
                          {modoPermissao === 'cargos'
                            ? 'As permissões serão aplicadas a todos os policiais que possuem este cargo.'
                            : 'As permissões concedidas ao grupo acumulam-se com as permissões do cargo do militar.'}
                        </p>
                      </div>
                      ...
                      <div className="w-full md:max-w-xs">
                        {modoPermissao === 'cargos' ? (
                          <select
                            ...
                            className="w-full bg-zinc-950 border border-zinc-800 text-xs text-zinc-100 px-3.5 py-2.5 rounded focus:border-amber-500 outline-none font-tactical"
                          >
                          ...
                          <select
                            value={gGrupoSelecionado}
                            onChange={(e) => carregarPermissoesGrupo(e.target.value)}
                            className="w-full bg-zinc-950 border border-zinc-800 text-xs text-zinc-100 px-3.5 py-2.5 rounded focus:border-amber-500 outline-none font-tactical"
                          >
                    ...
                    {((modoPermissao === 'cargos' && gCargoSelecionado) || (modoPermissao === 'grupos' && gGrupoSelecionado)) ? (
                      <form
                        onSubmit={modoPermissao === 'cargos' ? lidarComSalvarPermissoesCargo : lidarComSalvarPermissoesGrupo}
                        className="space-y-6"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* Left Column: Allowed Tabs */}
                          <div className="bg-zinc-950/80 border border-zinc-900 rounded-lg p-5 space-y-5">
                            ...
                            {/* Section: Setor Administrativo */}
                            <div className="space-y-2">
                              <div className="flex items-center gap-1.5 text-[11px] font-tactical font-bold text-red-500 uppercase tracking-wider">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pl-1">
                                {[
                                  ...
                                  { id: 'apostilas', label: 'Gerenciar Apostilas' },
                                ].map((aba) => (
                                  <label key={aba.id} className="flex items-center gap-2.5 p-2 bg-red-950/20 border border-red-900/40 rounded cursor-pointer hover:bg-red-950/40 hover:border-red-800 transition-all select-none">
                                    <input
                                      type="checkbox"
                                    ...
                                    />
                                    <span className="text-xs font-tactical text-zinc-200">{aba.label}</span>
                                  </label>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="space-y-2 border-t border-zinc-900 pt-3">
                              <div className="flex items-center gap-1.5 text-[11px] font-tactical font-bold text-zinc-400 uppercase tracking-wider">
                                <Sliders className="w-3.5 h-3.5 text-amber-500" />
                                <span>Outras Abas do Painel</span>
                              </div>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pl-1">
                                {[
                        ...
                        >
                          ...
                          {modoPermissao === 'cargos'
                            ...
                            : `SALVAR AUTORIZAÇÕES DO GRUPO: ${(grupos.find(g => g.id === gGrupoSelecionado)?.nome || gGrupoSelecionado).toUpperCase()}`}
                        </button>
                      </form>
                    ) : (
                      <div className="text-center py-16 text-zinc-650 font-tactical">
                        ...
                        <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">
                          {modoPermissao === 'cargos' ? 'Selecione um cargo acima para configurar' : 'Selecione um grupo acima para configurar'}
                        </p>
                ...
                {/* Sub Aba: CARGOS DE GRUPOS */}
                {gSubAba === 'cargos_grupos' && (
                  <div className="bg-zinc-950/60 border border-zinc-900 rounded-lg p-5 space-y-6">
                    <div className="border-b border-zinc-900 pb-4 space-y-1">
                      <h4 className="text-sm font-tactical font-bold text-zinc-300 tracking-wider uppercase flex items-center gap-2">
                        <Shield className="w-4 h-4 text-amber-500" />
                        Gerenciador de Cargos e Títulos de Grupos / Departamentos
                      </h4>
                      <p className="text-xs text-zinc-500 font-tactical">
                        Crie quantos cargos quiser para cada grupo e atribua policiais aos seus respectivos cargos por Nick. Esses cargos serão exibidos automaticamente nos perfis dos militares.
                      </p>
                    </div>

                    {/* Form: Criar Novo Cargo de Grupo */}
                    <form onSubmit={lidarComCriarCargoGrupo} className="bg-zinc-900/40 border border-zinc-850 p-4 rounded-lg space-y-4">
                      <h5 className="text-xs font-tactical font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                        <Plus className="w-4 h-4" />
                        Criar Novo Cargo de Grupo
                      </h5>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] text-zinc-400 font-tactical uppercase mb-1">
                            Selecione o Grupo / Departamento *
                          </label>
                          <select
                            value={cgGrupoId}
                            onChange={(e) => setCgGrupoId(e.target.value)}
                            className="w-full bg-zinc-950 border border-zinc-800 focus:border-amber-500 text-xs text-zinc-200 px-3 py-2 rounded outline-none font-tactical"
                            required
                          >
                            <option value="">-- Selecione um Grupo --</option>
                            {grupos.map((g) => (
                              <option key={g.id} value={g.id}>
                                {g.nome} [{g.sigla.toUpperCase()}]
                              </option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div>
                          <label className="block text-[10px] text-zinc-400 font-tactical uppercase mb-1">
                            Nome do Cargo (Ex: Líder, Sub-Comandante, Instrutor, Membro de Honra) *
                          </label>
                          <input
                            type="text"
                            value={cgNomeCargo}
                            onChange={(e) => setCgNomeCargo(e.target.value)}
                            placeholder="Ex: Líder de Grupo, Comandante, Supervisor..."
                            className="w-full bg-zinc-950 border border-zinc-800 focus:border-amber-500 text-xs text-zinc-200 px-3 py-2 rounded outline-none font-tactical"
                            required
                          />
                        </div>
                      ...
                      <div className="flex justify-end pt-2">
                        <button
                          type="submit"
                          className="px-5 py-2 bg-gradient-to-r from-amber-950 via-amber-700 to-amber-950 hover:from-amber-800 hover:to-amber-800 text-white font-tactical font-bold text-xs tracking-wider rounded transition-all cursor-pointer flex items-center gap-1.5 shadow-md"
                        >
                          <Plus className="w-4 h-4" />
                          <span>CRIAR CARGO DE GRUPO</span>
                        </button>
                      </div>
                    </form>

                    {/* Section: List of Groups and Their Cargos */}
                    <div className="space-y-4 pt-2">
                      <div className="flex items-center justify-between border-b border-zinc-900 pb-2">
                        <h5 className="text-xs font-tactical font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                          <Users className="w-4 h-4 text-amber-500" />
                          Cargos Cadastrados por Grupo ({grupos.reduce((acc, g) => acc + (g.cargos?.length || 0), 0)})
                        </h5>
                      </div>

                      {grupos.length === 0 ? (
                        <div className="p-8 text-center bg-zinc-900/30 border border-zinc-850 rounded-lg text-zinc-500 font-tactical text-xs">
                          Nenhum grupo cadastrado no site ainda. Crie grupos na aba "Grupos e Departamentos" primeiro.
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {grupos.map((grp) => {
                            const badgeUrl = obterBadgeUrl(grp.urlImagem);
                            const cargosDoGrupo = grp.cargos || [];

                            return (
                              <div key={grp.id} className="bg-zinc-950 border border-zinc-850 rounded-lg p-4 space-y-4">
                                {/* Group Header */}
                                <div className="flex items-center justify-between border-b border-zinc-900 pb-3 flex-wrap gap-2">
                                  <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 bg-zinc-900 border border-zinc-800 rounded flex items-center justify-center p-0.5 shrink-0">
                                      <img
                                        src={badgeUrl}
                                        alt={grp.nome}
                                    ...
                                    <div>
                                      <div className="flex items-center gap-2">
                                        <h6 className="text-sm font-tactical font-bold text-zinc-100">{grp.nome}</h6>
                                        <span className="text-[10px] font-mono text-zinc-400 bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-800">
                                          [{grp.sigla.toUpperCase()}]
                                        </span>
                                      </div>
                                      <span className="text-[10px] text-zinc-500 font-tactical block">
                                        {cargosDoGrupo.length} cargo(s) cadastrado(s) • {grp.membros?.length || 0} membro(s) no grupo
                                      </span>
                                    </div>
                                  </div>
                                </div>

                                {/* List of Cargos inside this Group */}
                                {cargosDoGrupo.length === 0 ? (
                                  <p className="text-xs text-zinc-600 italic font-tactical py-1">
                                    Nenhum cargo criado para este grupo ainda.
                                  </p>
                                ) : (
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {cargosDoGrupo.map((cargo) => (
                                      <div key={cargo.id} className="bg-zinc-900/40 border border-zinc-800/80 rounded-lg p-3 space-y-3 flex flex-col justify-between">
                                        <div>
                                          <div className="flex items-center justify-between gap-2 border-b border-zinc-850 pb-2">
                                            <span className="text-xs font-tactical font-bold text-amber-400 flex items-center gap-1.5 uppercase">
                                              <Shield className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                                              {cargo.nome}
                                            </span>
                                            <button
                                              type="button"
                                              onClick={() => lidarComExcluirCargoGrupo(grp.id, cargo.id, cargo.nome)}
                                              className="text-red-500 hover:text-red-400 p-1 bg-red-950/20 hover:bg-red-950/50 border border-red-900/30 rounded transition-all cursor-pointer"
                                              title="Excluir Cargo"
                                            >
                                              <Trash2 className="w-3 h-3" />
                                            </button>
                                          ...
                                          {/* List of assigned officers in this Cargo */}
                                          <div className="pt-2 space-y-1.5">
                                            <span className="text-[10px] text-zinc-500 font-tactical block uppercase">
                                              Policiais neste Cargo ({cargo.membros?.length || 0}):
                                            </span>

                                            {(!cargo.membros || cargo.membros.length === 0) ? (
                                              <p className="text-[11px] text-zinc-600 italic font-tactical">
                                                Nenhum policial adicionado a este cargo ainda.
                                              </p>
                                            ) : (
                                              <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto custom-scrollbar pt-1">
                                                {cargo.membros.map((nickMembro) => (
                                                  <div
                                                    key={nickMembro}
                                                    className="flex items-center gap-1 px-2 py-0.5 bg-zinc-950 border border-amber-900/30 text-zinc-200 rounded text-[11px] font-tactical"
                                                  >
                                                    <span className="font-semibold">{nickMembro}</span>
                                                    <button
                                                      type="button"
                                                      onClick={() => lidarComRemoverPessoaCargoGrupo(grp.id, cargo.id, nickMembro)}
                                                      className="text-zinc-500 hover:text-red-400 p-0.5 transition-colors cursor-pointer"
                                                      title={`Remover ${nickMembro} deste cargo`}
                                                    >
                                                      <X className="w-3 h-3" />
                                                    </button>
                                                  </div>
                                        ...
                                        {/* Input to add a person by Nick to this Cargo */}
                                        <div className="pt-2 border-t border-zinc-850 flex items-center gap-1.5">
                                          <input
                                            type="text"
                                            list="lista-policiais-cargos"
                                            placeholder="Nick do Policial..."
                                            value={cgNickPessoa[cargo.id] || ''}
                                            onChange={(e) => setCgNickPessoa(prev => ({ ...prev, [cargo.id]: e.target.value }))}
                                            onKeyDown={(e) => {
                                              if (e.key === 'Enter') {
                                                e.preventDefault();
                                                lidarComAdicionarPessoaCargoGrupo(grp.id, cargo.id);
                                              }
                                            }}
                                            className="flex-1 bg-zinc-950 border border-zinc-800 focus:border-amber-500 text-xs text-zinc-200 px-2.5 py-1.5 rounded outline-none font-tactical"
                                          />
                                          <button
                                            type="button"
                                            onClick={() => lidarComAdicionarPessoaCargoGrupo(grp.id, cargo.id)}
                                            className="px-3 py-1.5 bg-amber-950/80 hover:bg-amber-900 border border-amber-800 text-amber-300 font-tactical font-bold text-[11px] rounded transition-all cursor-pointer shrink-0 flex items-center gap-1"
                                          >
                                            <Plus className="w-3.5 h-3.5" />
                                            <span>ADICIONAR</span>
                                          </button>
                      ...
                      {/* Datalist for officer autocomplete */}
                      <datalist id="lista-policiais-cargos">
                        {(Object.values(policiais) as Policial[]).map(p => (
                            ...
                            .map(p => (
                              <div key={p.nick} className="py-2.5 flex items-center justify-between text-xs">
                                ...
                                <div className="flex gap-1.5 shrink-0">
                                  ...
                                  {p.nick.toLowerCase() !== 'admin' && p.nick.toLowerCase() !== 'policecia' && (
                                    <button
                                      type="button"
            ...
            >
              ...
              <div className="p-5 space-y-6 flex-1">
                ...
                <form onSubmit={lidarComSalvarGrupo} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-[10px] text-zinc-500 font-tactical mb-1 uppercase tracking-wider">Imagem / Logo do Grupo (Link Direct, Discord ou Código Habbo)</label>
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded flex items-center justify-center p-1 shrink-0 overflow-hidden">
                        <img
                          src={obterBadgeUrl(grupoUrlImagem)}
                      ...
                      <input
                        ...
                        placeholder="Cole o Link da Imagem (Ex: Discord, Imgur) ou código de emblema Habbo..."
