import { Policial, PatenteCategoria, Patente, Aviso, RecrutaTeste, Movimentacao, Grupo, Treinamento, AplicacaoTreinamento, EnvioXP, ConfiguracaoSite, Apostila, PostAbaCustomizada } from '../types';
import { obterSupabaseClient } from './supabase';

// Marca o instante da última escrita local em memória
let ultimaEscritaLocalEm = 0;

export function marcarEscritaLocal() {
  ultimaEscritaLocalEm = Date.now();
}

export function houveEscritaLocalRecente(janelaMs: number = 2000): boolean {
  return (Date.now() - ultimaEscritaLocalEm) < janelaMs;
}

// Helpers de criptografia para auxílio nas senhas
export function criptografarTexto(texto: string): string {
  if (!texto) return '';
  const key = "CIA_SECURE_KEY_2026";
  let result = "";
  for (let i = 0; i < texto.length; i++) {
    const charCode = texto.charCodeAt(i) ^ key.charCodeAt(i % key.length);
...
export function descriptografarTexto(encoded: string): string {
  if (!encoded) return '';
  try {
...
// Armazenamento em memória principal (Sem salvar em localStorage)
let dbPoliciais: Record<string, Policial & { senhaHex: string }> = {};
let dbAvisos: Aviso[] = [];
let dbTestes: RecrutaTeste[] = [];
let dbMovimentacoes: Movimentacao[] = [];
let dbCategorias: PatenteCategoria[] = [];
let dbPatentes: Patente[] = [];
let dbGrupos: Grupo[] = [];
let dbTreinamentos: Treinamento[] = [];
let dbAplicacoesTreinamentos: AplicacaoTreinamento[] = [];
let dbXpEnviados: EnvioXP[] = [];
let dbConfiguracaoSite: ConfiguracaoSite = { ...CONFIGURACAO_PADRAO };
let dbAdvertencias: any[] = [];
let dbExoneracoes: any[] = [];
let dbSolicitacoesCadastro: any[] = [];
let dbPostsAbasCustomizadas: PostAbaCustomizada[] = [];
let dbApostilas: Apostila[] = [];
let dbInicializado = false;

// Funções para atualização direta do cache em memória a partir do Supabase
export function definirPoliciais(pols: Record<string, Policial & { senhaHex: string }>) {
  dbPoliciais = pols;
}
export function definirAvisos(avisos: Aviso[]) {
  dbAvisos = avisos;
}
export function definirTestes(testes: RecrutaTeste[]) {
  dbTestes = testes;
}
export function definirMovimentacoes(movs: Movimentacao[]) {
  dbMovimentacoes = movs;
}
export function definirCategorias(cats: PatenteCategoria[]) {
  dbCategorias = cats;
}
export function definirPatentes(pats: Patente[]) {
  dbPatentes = pats;
}
export function definirGrupos(grupos: Grupo[]) {
  dbGrupos = grupos;
}
export function definirTreinamentos(treins: Treinamento[]) {
  dbTreinamentos = treins;
}
export function definirAplicacoesTreinamentos(apls: AplicacaoTreinamento[]) {
  dbAplicacoesTreinamentos = apls;
}
export function definirXpEnviados(xps: EnvioXP[]) {
  dbXpEnviados = xps;
}
export function definirConfiguracaoSite(config: ConfiguracaoSite) {
  dbConfiguracaoSite = config;
}
export function definirAdvertencias(advs: any[]) {
  dbAdvertencias = advs;
}
export function definirExoneracoes(exos: any[]) {
  dbExoneracoes = exos;
}
export function definirSolicitacoesCadastro(sols: any[]) {
  dbSolicitacoesCadastro = sols;
}
export function definirPostsAbasCustomizadas(posts: PostAbaCustomizada[]) {
  dbPostsAbasCustomizadas = posts;
}
export function definirApostilas(apostilas: Apostila[]) {
  dbApostilas = apostilas;
}
...
export function inicializarDB() {
  ...
  delete dbPoliciais['admin'];
  dbPoliciais['policecia'] = accountAdmin;
  delete dbPoliciais['78k'];
  ...
  dbConfiguracaoSite = { ...CONFIGURACAO_PADRAO };
  dbInicializado = true;
}
...
export function obterPoliciais(): Record<string, Policial & { senhaHex: string }> {
  inicializarDB();
  if (dbPoliciais['policecia']) {
    dbPoliciais['policecia'].pontosPromocao = 0;
  }
  return dbPoliciais;
}

export function salvarPolicial(policial: Policial & { senhaHex?: string }) {
  inicializarDB();
  const nickMin = policial.nick.toLowerCase().trim();
  const existente = dbPoliciais[nickMin] || {} as Partial<Policial & { senhaHex: string }>;
  const completo = {
    ...existente,
    ...policial,
    senhaHex: policial.senhaHex || existente.senhaHex || criptografarTexto('123456')
  };
  if (nickMin === 'admin' || nickMin === 'policecia') {
    completo.pontosPromocao = 0;
  }
  dbPoliciais[nickMin] = completo;
  sincronizarDadoEspecifico('policiais', completo);
  return completo;
}

export function filtrarPoliciaisVisiveis<T extends { nick: string }>(lista: T[]): T[] {
  return lista.filter(p => p.nick.toLowerCase().trim() !== 'admin' && p.nick.toLowerCase().trim() !== 'policecia');
}
...
export function alterarSenhaPolicial(nick: string, novaSenhaHex: string): { sucesso: boolean; mensagem: string } {
  ...
  policiais[nickMin].senhaHex = criptografarTexto(novaSenhaHex);
...
export function registrarPolicial(
  ...
  const nova = {
    ...
    nick: nick.trim(),
    senhaHex: criptografarTexto(senhaHex),
  ...
  const embed = {
    title: '🟡 NOVA SOLICITAÇÃO DE ALISTAMENTO PENDENTE',
    description: `O militar **@${nova.nick}** enviou uma solicitação de cadastro/alistamento pelo site e aguarda homologação de um Oficial/Administrador.`,
    color: 16705372,
    fields: [
      { name: '👤 Policial', value: `@${nova.nick}`, inline: true },
      { name: '🏷️ Habbo', value: `[${nova.avatarHabbo}](https://www.habblet.com.br/profile/${nova.avatarHabbo})`, inline: true },
      { name: '📅 Data/Hora', value: nova.dataSolicitacao, inline: true },
      { name: '📍 Localização', value: nova.cidade || 'Não detectada', inline: true },
      { name: '🌐 Endereço IP', value: `\`${nova.ip || 'Não detectado'}\``, inline: true }
    ],
    thumbnail: {
      url: nova.fotoUrl || `https://api.radiohabblet.com.br/imager?user=${encodeURIComponent(nova.avatarHabbo)}&gesture=std&direction=3&head_direction=3&headonly=false&size=m`
    },
    footer: { text: 'Polícia CIA • Controle de Admissões' },
  ...
  try {
    ...
    console.error('Erro ao enviar log de nova solicitação:', err);
  }
...
export function aceitarSolicitacaoCadastro(id: string, autorNick: string): { sucesso: boolean; mensagem: string } {
  ...
  const patenteSoldado = patentes.find(p => p.nome.toLowerCase().trim() === 'soldado');
  const cargoInicial = patenteSoldado ? patenteSoldado.nome : 'Soldado';
  const categoriaIdInicial = patenteSoldado ? patenteSoldado.categoriaId : undefined;
  ...
  sol.status = 'aprovado';
  sol.avaliadorNick = autorNick;
  sol.dataAvaliacao = new Date().toLocaleDateString('pt-BR') + ' ' + new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });

  sincronizarDadoEspecifico('solicitacoes_cadastro', sol);
  ...
  const embed = {
    ...
    fields: [
      ...
      { name: '🏷️ Habbo', value: `[${sol.avatarHabbo}](https://www.habblet.com.br/profile/${sol.avatarHabbo})`, inline: true },
      { name: '💂 Cargo Inicial', value: cargoInicial, inline: true },
    ...
    thumbnail: {
      url: sol.fotoUrl || `https://api.radiohabblet.com.br/imager?user=${encodeURIComponent(sol.avatarHabbo)}&gesture=std&direction=3&head_direction=3&headonly=false&size=m`
    },
    footer: { text: 'Polícia CIA • Controle de Admissões' },
    timestamp: new Date().toISOString()
  };
...
export function recusarSolicitacaoCadastro(id: string, autorNick: string): { sucesso: boolean; mensagem: string } {
  const solicitacoes = obterSolicitacoesCadastro();
  const index = solicitacoes.findIndex(s => s.id === id);
  if (index < 0) {
    return { sucesso: false, mensagem: 'Solicitação não encontrada.' };
  ...
  const embed = {
    title: '🔴 SOLICITAÇÃO DE ALISTAMENTO REPROVADA',
    description: `O recrutamento de **@${sol.nick}** foi recusado/reprovado pelo administrador no painel.`,
    color: 15158332,
    fields: [
      { name: '👤 Policial', value: `@${sol.nick}`, inline: true },
      { name: '🏷️ Habbo', value: `[${sol.avatarHabbo}](https://www.habblet.com.br/profile/${sol.avatarHabbo})`, inline: true },
      { name: '🔐 Recusado Por', value: `@${autorNick}`, inline: true },
      { name: '📅 Data', value: sol.dataSolicitacao, inline: true }
    ],
    thumbnail: {
      url: sol.fotoUrl || `https://api.radiohabblet.com.br/imager?user=${encodeURIComponent(sol.avatarHabbo)}&gesture=std&direction=3&head_direction=3&headonly=false&size=m`
    },
  ...
  return { sucesso: true, mensagem: `Solicitação de @${sol.nick} reprovada com sucesso.` };
}
...
export function loginPolicial(nick: string, senhaHex: string): { sucesso: boolean; policial?: Policial; mensagem: string } {
  ...
  const match = policiais[nickMin].senhaHex === senhaHex || 
                policiais[nickMin].senhaHex === criptografarTexto(senhaHex);
...
export function atualizarPerfilPolicial(nick: string, biografia: string, avatarHabbo: string, fotoUrl?: string): Policial {
  const policiais = obterPoliciais();
  const nickMin = nick.toLowerCase().trim();
  ...
  if (policiais[nickMin]) {
    ...
    if (fotoUrl !== undefined) {
      policiais[nickMin].fotoUrl = fotoUrl;
    }
...
export function formatarDataHoraAtual(): string {
...
export function alterarPatentePolicial(
  ...
  if (policiais[nickMin]) {
    ...
    const patentesList = obterPatentes();
    const matchingPat = patentesList.find(p => p.nome.toLowerCase().trim() === novoCargo.toLowerCase().trim());
    if (matchingPat) {
      policiais[nickMin].categoriaId = matchingPat.categoriaId;
    }
...
export function adicionarPontos(nickPolicial: string, pontos: number): { sucesso: boolean; policial?: Policial } {
  ...
  if (policiais[nickMin]) {
    ...
    if (policiais[nickMin].pontosPromocao < 0) {
      policiais[nickMin].pontosPromocao = 0;
    }
    if (nickMin === 'admin') {
      policiais[nickMin].pontosPromocao = 0;
    }
...
export function excluirAviso(id: string): void {
  dbAvisos = dbAvisos.filter(a => a.id !== id);
  sincronizarDadoEspecifico('avisos', id, 'delete');
}
...
export function salvarTeste(teste: Omit<RecrutaTeste, 'id' | 'data'>): RecrutaTeste {
  ...
  if (policiais[examMin]) {
    policiais[examMin].pontosPromocao += 5;
    policiais[examMin].presencas += 1;
    if (examMin === 'admin') {
      policiais[examMin].pontosPromocao = 0;
    }
...
// Categorias e Patentes
export function obterCategorias(): PatenteCategoria[] {
  inicializarDB();
  return dbCategorias.sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
}
...
export function excluirCategoria(id: string): void {
  ...
  const paraExcluir = dbPatentes.filter(p => p.categoriaId === id);
  dbPatentes = dbPatentes.filter(p => p.categoriaId !== id);
...
export function salvarPatente(patente: Patente): Patente {
  const index = dbPatentes.findIndex(p => p.id === patente.id);
  if (index >= 0) {
    dbPatentes[index] = patente;
  } else {
    dbPatentes.push(patente);
  }
...
export function excluirPatente(id: string): void {
  dbPatentes = dbPatentes.filter(p => p.id !== id);
...
export function salvarGrupo(grupo: Grupo): Grupo {
  const index = dbGrupos.findIndex(g => g.id === grupo.id);
  if (index >= 0) {
    dbGrupos[index] = grupo;
  } else {
    dbGrupos.push(grupo);
  }
...
export function excluirGrupo(id: string): void {
  dbGrupos = dbGrupos.filter(g => g.id !== id);
...
export function obterTreinamentos(): Treinamento[] {
  inicializarDB();
  return [...dbTreinamentos].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
}

export function salvarTreinamento(t: Treinamento): Treinamento {
  if (typeof t.ordem !== 'number') {
    t.ordem = dbTreinamentos.length > 0 ? Math.max(...dbTreinamentos.map(item => item.ordem ?? 0)) + 1 : 1;
  }
  const index = dbTreinamentos.findIndex(item => item.id === t.id);
  if (index >= 0) {
    dbTreinamentos[index] = t;
  } else {
    dbTreinamentos.push(t);
  }
...
export function excluirTreinamento(id: string): void {
  dbTreinamentos = dbTreinamentos.filter(t => t.id !== id);
  sincronizarDadoEspecifico('treinamentos', id, 'delete');
}

export function obterAplicacoesTreinamentos(): AplicacaoTreinamento[] {
  inicializarDB();
  return dbAplicacoesTreinamentos;
}

export function salvarAplicacaoTreinamento(a: AplicacaoTreinamento): AplicacaoTreinamento {
  const index = dbAplicacoesTreinamentos.findIndex(item => item.id === a.id);
  if (index >= 0) {
    dbAplicacoesTreinamentos[index] = a;
  } else {
    dbAplicacoesTreinamentos.push(a);
  }
...
export function excluirAplicacaoTreinamento(id: string): void {
  dbAplicacoesTreinamentos = dbAplicacoesTreinamentos.filter(a => a.id !== id);
...
export function salvarEnvioXP(envio: EnvioXP): EnvioXP {
  const index = dbXpEnviados.findIndex(e => e.id === envio.id);
  if (index >= 0) {
    dbXpEnviados[index] = envio;
  } else {
    dbXpEnviados.unshift(envio);
  }
...
export function excluirEnvioXP(id: string): void {
  const envio = dbXpEnviados.find(e => e.id === id);
  if (envio) {
    // Revert XP granted to recipients
    envio.destinatarios.forEach((nick) => {
      adicionarPontos(nick, -envio.valor);
    });
  }
  dbXpEnviados = dbXpEnviados.filter(e => e.id !== id);
  sincronizarDadoEspecifico('xp_enviados', id, 'delete');
}
...
export async function sincronizarDadoEspecifico(tabela: string, dados: any, action: 'upsert' | 'delete' = 'upsert') {
  marcarEscritaLocal();
  try {
    ...
    if (action === 'delete') {
      if (tabela === 'policiais') {
        const nickClean = String(dados).trim();
        const { error: errIlike } = await (supabase.from('policiais').delete() as any).ilike('nick', nickClean);
        if (errIlike) {
          console.warn(`[Supabase Sync] Delete for policiais ilike (${nickClean}):`, errIlike.message || errIlike);
          await supabase.from('policiais').delete().eq('nick', nickClean);
        }
      } else if (tabela === 'solicitacoes_cadastro_by_nick') {
        const nickClean = String(dados).trim();
        const { error: errIlike } = await (supabase.from('solicitacoes_cadastro').delete() as any).ilike('nick', nickClean);
        if (errIlike) {
          console.warn(`[Supabase Sync] Delete for solicitacoes_cadastro ilike (${nickClean}):`, errIlike.message || errIlike);
          await supabase.from('solicitacoes_cadastro').delete().eq('nick', nickClean);
        }
      } else {
        const keyCol = 'id';
        const { error } = await supabase.from(tabela).delete().eq(keyCol, dados);
        if (error) {
          console.warn(`[Supabase Sync] Delete for ${tabela}:`, error.message || error);
        }
      }
      return;
    }
    ...
    if (tabela === 'configuracao_site') {
      payload = {
        ...
        grupos_permissoes: dados.gruposPermissoes || {},
        webhook_discord: dados.webhookDiscord || null,
        cor_tema: dados.corTema || null,
        login_background_url: dados.loginBackgroundUrl || null,
        link_suporte: dados.linkSuporte || null,
        link_discord: dados.linkDiscord || null,
        webhook_avisos: dados.webhookAvisos || null,
        webhook_movimentacoes: dados.webhookMovimentacoes || null,
        webhook_advertencias: dados.webhookAdvertencias || null,
        webhook_exoneracoes: dados.webhookExoneracoes || null,
        webhook_discord_entradas: dados.webhookDiscordEntradas || null,
        webhook_backup: dados.webhookBackup || null,
        ultimo_backup: dados.ultimoBackup || null,
        abas_customizadas: dados.abasCustomizadas || []
      };
    } else if (tabela === 'policiais') {
      const patList = obterPatentes();
      const cargoAlvo = String(dados.cargo || '').toLowerCase().trim();
      const matchingPat = cargoAlvo ? patList.find(p => String(p.nome || '').toLowerCase().trim() === cargoAlvo) : undefined;
      const resolvedCatId = dados.categoriaId || (matchingPat ? matchingPat.categoriaId : null);
      ...
      if (String(dados.nick).toLowerCase().trim() === 'admin') {
        payload.pontos_promocao = 0;
      }
      ...
      payload = {
        ...
        cor_fundo: dados.corFundo || 'default',
        negrito: !!dados.negrito,
        italico: !!dados.italico,
        marcar_everyone: !!dados.marcarEveryone,
        posicao_imagem: dados.posicaoImagem || 'topo',
        imagem_fundo_url: dados.imagemFundoUrl || null,
        template_noticia: dados.templateNoticia || 'padrao'
      };
      ...
      payload = {
        ...
        cargos_que_podem_ver: dados.cargosQuePodemVer || ['todos'],
        permissoes_avaliadores: dados.permissoesAvaliadores || ['todos'],
        ordem: typeof dados.ordem === 'number' ? dados.ordem : 0
      };
      ...
      payload = {
        ...
        status_aprovacao: dados.statusAprovacao || 'aprovado',
        avaliador: dados.avaliador || null,
        data_avaliacao: dados.dataAvaliacao || null,
        motivo_avaliacao: dados.motivoAvaliacao || null
      };
    ...
    } else if (tabela === 'solicitacoes_cadastro') {
      payload = {
        id: dados.id,
        nick: dados.nick,
        senha_hex: dados.senhaHex,
        ...
        ip: dados.ip || 'Não detectado',
        cidade: dados.cidade || 'Não detectada',
        foto_url: dados.fotoUrl || null,
        status: dados.status || 'pendente',
        avaliador_nick: dados.avaliadorNick || null,
        data_avaliacao: dados.dataAvaliacao || null
      };
    } else if (tabela === 'posts_abas_customizadas') {
      payload = {
        id: dados.id,
        aba_id: dados.abaId,
        titulo: dados.titulo,
        ...
        autor: dados.autor,
        data: dados.data,
        file_url: dados.fileUrl || null,
        tipo: dados.tipo || 'documento'
      };
    } else if (tabela === 'apostilas') {
      payload = {
        id: dados.id,
        titulo: dados.titulo,
        descricao: dados.descricao || null,
        ...
        data_criacao: dados.dataCriacao || null
      };
    }

    if (payload) {
      if ((tabela === 'patentes' || tabela === 'policiais') && payload.categoria_id) {
        const catList = obterCategorias();
        const parentCat = catList.find(c => c.id === payload.categoria_id);
        const catToSync = parentCat || {
          id: payload.categoria_id,
        ...
        await supabase.from('categorias').upsert({
          id: catToSync.id,
          nome: catToSync.nome,
          subtitulo: catToSync.subtitulo || null,
          ordem: typeof catToSync.ordem === 'number' ? catToSync.ordem : 0
        });
        const { error } = await supabase.from(tabela).upsert(payload);
        if (error) {
          console.warn(`[Supabase Sync] Upsert for ${tabela}:`, error.message || error);
        }
        return;
      }
    ...
    console.warn(`[Supabase Sync] Falha ao sincronizar "${tabela}" com o Supabase:`, error?.message || error);
  }
}

export function obterConfiguracaoSite(): ConfiguracaoSite {
  inicializarDB();
  return dbConfiguracaoSite;
}
...
export function obterAdvertencias(): any[] {
  inicializarDB();
  return dbAdvertencias;
}

export function salvarAdvertencia(adv: any): any {
  const index = dbAdvertencias.findIndex(a => a.id === adv.id);
  const isNew = index < 0;
  if (index >= 0) {
    dbAdvertencias[index] = adv;
  } else {
    dbAdvertencias.unshift(adv);
  }
...
export function excluirAdvertencia(id: string): void {
  const aSerExcluido = dbAdvertencias.find(a => a.id === id);
  dbAdvertencias = dbAdvertencias.filter(a => a.id !== id);
...
export function obterExoneracoes(): any[] {
  inicializarDB();
  return dbExoneracoes;
}

export function salvarExoneracao(exo: any): any {
  const index = dbExoneracoes.findIndex(e => e.id === exo.id);
  if (index >= 0) {
    dbExoneracoes[index] = exo;
  } else {
    dbExoneracoes.unshift(exo);
  }
...
export function exonerarPolicialNoBanco(nickPolicial: string, motivo: string, autor: string, tipo: 'demissao' | 'exoneracao'): boolean {
  inicializarDB();
  if (!nickPolicial || !nickPolicial.trim()) return false;

  const nickMin = nickPolicial.toLowerCase().trim();
  const keyEncontrada = Object.keys(dbPoliciais).find(k => k.toLowerCase().trim() === nickMin);
  
  const polOriginal = keyEncontrada ? dbPoliciais[keyEncontrada] : null;
  const realNick = polOriginal ? polOriginal.nick : nickPolicial.trim();

  // 1. Remove from local memory dictionary
  if (keyEncontrada) delete dbPoliciais[keyEncontrada];
  if (dbPoliciais[nickMin]) delete dbPoliciais[nickMin];

  // 2. Delete from Supabase with case-insensitive ilike
  sincronizarDadoEspecifico('policiais', realNick, 'delete');
  if (realNick.toLowerCase() !== nickMin) {
    sincronizarDadoEspecifico('policiais', nickMin, 'delete');
  }

  // 3. Remove all registration requests (solicitacoes_cadastro) for this officer from memory & Supabase
  const solicitacoesASeremRemovidas = dbSolicitacoesCadastro.filter(
    s => s.nick.toLowerCase().trim() === nickMin || s.nick.toLowerCase().trim() === realNick.toLowerCase()
  );
  dbSolicitacoesCadastro = dbSolicitacoesCadastro.filter(
    s => s.nick.toLowerCase().trim() !== nickMin && s.nick.toLowerCase().trim() !== realNick.toLowerCase()
  );
  for (const sol of solicitacoesASeremRemovidas) {
    sincronizarDadoEspecifico('solicitacoes_cadastro', sol.id, 'delete');
  }
  sincronizarDadoEspecifico('solicitacoes_cadastro_by_nick', realNick, 'delete');
  if (realNick.toLowerCase() !== nickMin) {
    sincronizarDadoEspecifico('solicitacoes_cadastro_by_nick', nickMin, 'delete');
  }

  // 3. Save Exoneration record
  const exo = {
    id: 'exo_' + Math.random().toString(36).substr(2, 9),
    nickPolicial: realNick,
    motivo: motivo,
    autor: autor,
  ...
  // 4. Save Public Announcement
  const avisos = obterAvisos();
  const novoAviso: Aviso = {
    id: Date.now().toString(),
    titulo: `${tipo === 'exoneracao' ? 'Exoneração' : 'Demissão'}: @${realNick}`,
    conteudo: `O policial @${realNick} foi desligado da corporação (${tipo === 'exoneracao' ? 'Exonerado' : 'Demitido'}). Autor: @${autor}. Motivo: ${motivo}`,
    autor,
    data: formatarDataHoraAtual(),
    tipo: 'geral'
  };
...
export async function enviarWebhookParaDiscord(
  ...
  try {
    ...
    let embed: any = { timestamp: new Date().toISOString() };
    ...
    if (tipo === 'aviso') {
      ...
      embed.color = dados.tipo === 'urgente' ? 15158332 : dados.tipo === 'promocao' ? 15844367 : 3447003;
      embed.fields = [
      ...
      embed.color = isPromo ? 3066993 : 15158332;
    ...
    fetch('/api/webhook/send', {
      ...
      body: JSON.stringify({ url: targetWebhookUrl, payload })
    }).catch(err => console.error('[Discord Webhook] Fetch exception:', err));
  } catch (error) {
...
export function enviarLogDiscordEntrada(embed: any) {
  try {
    ...
    fetch('/api/webhook/send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url: targetWebhookUrl, payload: { embeds: [embed] } })
    }).catch(err => console.error('[Discord Webhook Entradas] Fetch exception:', err));
  } catch (error) {
...
export function salvarApostila(apostila: Apostila): { sucesso: boolean; mensagem: string } {
  inicializarDB();
  const index = dbApostilas.findIndex(a => a.id === apostila.id);
  if (index >= 0) {
    dbApostilas[index] = apostila;
  } else {
    dbApostilas.push(apostila);
  }

  sincronizarDadoEspecifico('apostilas', apostila);
...
export function excluirApostila(id: string): { sucesso: boolean; mensagem: string } {
  dbApostilas = dbApostilas.filter(a => a.id !== id);
  sincronizarDadoEspecifico('apostilas', id, 'delete');
...
export async function limparEResetarBancoDeDados(): Promise<{ sucesso: boolean; mensagem: string }> {
  try {
    ...
    dbAdvertencias = [];
    dbExoneracoes = [];
    dbSolicitacoesCadastro = [];
    dbPostsAbasCustomizadas = [];
    dbApostilas = [];
    ...
    return { 
      sucesso: true, 
      mensagem: `Todos os dados foram limpos e reiniciados com sucesso${supabaseMsg}! A conta 'admin' (senha: admin#2026) foi recriada para acesso.` 
    };
  } catch (error: any) {
...
export function excluirPolicialPermanente(nickPolicial: string): boolean {
  if (!nickPolicial || !nickPolicial.trim()) return false;
  const nickMin = nickPolicial.toLowerCase().trim();
  const keyEncontrada = Object.keys(dbPoliciais).find(k => k.toLowerCase().trim() === nickMin);
  const realNick = keyEncontrada && dbPoliciais[keyEncontrada] ? dbPoliciais[keyEncontrada].nick : nickPolicial.trim();
  ...
  // Remove registration requests from memory & Supabase
  const solicitacoesASeremRemovidas = dbSolicitacoesCadastro.filter(
    s => s.nick.toLowerCase().trim() === nickMin || s.nick.toLowerCase().trim() === realNick.toLowerCase()
  );
  dbSolicitacoesCadastro = dbSolicitacoesCadastro.filter(
...
// Posts em abas customizadas
export function obterPostsAbasCustomizadas(): PostAbaCustomizada[] {
  inicializarDB();
  return dbPostsAbasCustomizadas;
}

export function salvarPostAbaCustomizada(post: PostAbaCustomizada): PostAbaCustomizada {
  const index = dbPostsAbasCustomizadas.findIndex(p => p.id === post.id);
  if (index >= 0) {
    dbPostsAbasCustomizadas[index] = post;
  } else {
    dbPostsAbasCustomizadas.unshift(post);
  }
  sincronizarDadoEspecifico('posts_abas_customizadas', post);
  return post;
}

export function deletarPostAbaCustomizada(id: string): boolean {
  const index = dbPostsAbasCustomizadas.findIndex(p => p.id === id);
  if (index >= 0) {
    dbPostsAbasCustomizadas.splice(index, 1);
    sincronizarDadoEspecifico('posts_abas_customizadas', id, 'delete');
    return true;
  }
  return false;
}
