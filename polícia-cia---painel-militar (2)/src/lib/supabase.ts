...
import { 
  ...
  definirAvisos, 
  definirTestes, 
  definirMovimentacoes, 
  definirCategorias, 
  definirPatentes, 
  definirGrupos, 
  definirTreinamentos, 
  definirAplicacoesTreinamentos, 
  definirXpEnviados, 
  definirConfiguracaoSite, 
  definirAdvertencias, 
  definirExoneracoes, 
  definirSolicitacoesCadastro, 
  definirPostsAbasCustomizadas, 
  definirApostilas,
  obterPoliciais,
  obterAvisos,
  obterTestes,
  ...
  obterGrupos,
  obterTreinamentos,
  obterAplicacoesTreinamentos,
  obterEnviosXP,
  obterConfiguracaoSite,
  obterAdvertencias,
  obterExoneracoes,
  obterSolicitacoesCadastro,
  obterPostsAbasCustomizadas,
  obterApostilas
} from './db';
...
export async function testarConexaoSupabase(url?: string, key?: string): Promise<boolean> {
  ...
  // Fallback for NexSocial server proxy environment
  try {
...
export async function safeUpsertClient(client: SupabaseClient, tableName: string, payload: any): Promise<{ data: any; error: any }> {
  ...
  while (attempts < maxAttempts) {
        ...
        if (colName && colName !== tableName) {
          ...
          if (Array.isArray(currentPayload)) {
            currentPayload = currentPayload.map((item: any) => {
              if (item && typeof item === 'object' && colName in item) {
                const copy = { ...item };
                delete copy[colName];
                keyRemoved = true;
                return copy;
          ...
          } else if (currentPayload && typeof currentPayload === 'object' && colName in currentPayload) {
    ...
    if (error.code === '23503' || errorMsg.toLowerCase().includes('foreign key')) {
      console.log(`[Client Safe Upsert] FK violation on "${tableName}". Ensuring parent records exist...`);
      const items = Array.isArray(currentPayload) ? currentPayload : [currentPayload];
      let autoCreatedParent = false;
      for (const item of items) {
        if (item && typeof item === 'object') {
          ...
          if (item.treinamento_id) {
            try {
              await client.from('treinamentos').upsert({
                id: item.treinamento_id,
                titulo: 'Treinamento Integrado',
                status: 'ativo'
              });
              autoCreatedParent = true;
            } catch (e) {
              console.warn('[Client Safe Upsert] Auto treinamento creation failed:', e);
            }
          }
        }
      }
      if (autoCreatedParent && attempts < 3) {
        attempts++;
        continue;
      }
    }
  ...
  return { data: null, error: { message: `Não foi possível salvar na tabela "${tableName}". Verifique se a estrutura no Supabase está atualizada.` } };
}

// Client provider supporting both direct client-side requests (Vercel) and server proxy fallback (NexSocial)
...
CREATE TABLE IF NOT EXISTS configuracao_site (
  ...
  grupos_permissoes JSONB DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS treinamentos (
  ...
  cargos_que_podem_ver JSONB DEFAULT '["todos"]',
...
-- Garantir colunas novas caso as tabelas já existam
ALTER TABLE configuracao_site ADD COLUMN IF NOT EXISTS logo_url VARCHAR;
ALTER TABLE configuracao_site ADD COLUMN IF NOT EXISTS logo_texto VARCHAR;
ALTER TABLE configuracao_site ADD COLUMN IF NOT EXISTS login_background_url VARCHAR;
ALTER TABLE configuracao_site ADD COLUMN IF NOT EXISTS grupos_permissoes JSONB DEFAULT '{}';
ALTER TABLE grupos ADD COLUMN IF NOT EXISTS cargos_grupo JSONB DEFAULT '[]';
ALTER TABLE treinamentos ADD COLUMN IF NOT EXISTS ordem INTEGER DEFAULT 0;
ALTER TABLE treinamentos ADD COLUMN IF NOT EXISTS cargos_que_podem_ver JSONB DEFAULT '["todos"]';
ALTER TABLE categorias ADD COLUMN IF NOT EXISTS ordem INTEGER DEFAULT 0;
ALTER TABLE patentes ADD COLUMN IF NOT EXISTS ordem INTEGER DEFAULT 0;
...
export const SQL_SCRIPT_MIGRAR = `-- SCRIPT DE MIGRAÇÃO SEGURO (NÃO APAGA NENHUM DADO EXISTENTE)
-- Este script apenas cria tabelas e colunas faltantes no Supabase com "IF NOT EXISTS".
-- Seus dados já salvos estão 100% seguros e NENHUMA tabela será apagada ou sobrescrita.

` + SQL_SCRIPT_GERAL;

// Quando forcarPull=false (sync automático periódico), cada tabela só é sobrescrita
// localmente se o Supabase retornar itens - isso evita apagar dados locais por causa
// de uma falha passageira de rede que devolva uma lista vazia.
// Quando forcarPull=true ("Baixar do banco" no painel), a lista local é sempre
// substituída pela do Supabase, inclusive quando ela vier vazia - assim registros
// apagados diretamente no banco também somem do painel ao forçar o download.
  ...
  try {
    ...
    if (data.configuracao_site && data.configuracao_site.length > 0) {
      ...
      const mappedConfig: ConfiguracaoSite = {
        ...
        gruposPermissoes: active.grupos_permissoes || {},
      ...
      definirConfiguracaoSite(mappedConfig);
    }

    // 2. Sync policiais
    if (Array.isArray(data.policiais) && (data.policiais.length > 0 || forcarPull)) {
      ...
      // Force-ensure policecia and 78k exist in the pulled list if not present
      if (!polRecord['policecia']) {
        polRecord['policecia'] = {
          nick: 'PoliceCIA',
          cargo: 'Diretor',
      ...
      if (polRecord['policecia']) {
        polRecord['policecia'].pontosPromocao = 0;
      }
      // Se a conta antiga 'admin' ou '78k' for retornada do banco Supabase, remove do objeto local
      if (polRecord['admin']) {
        delete polRecord['admin'];
      }
      if (polRecord['78k']) {
        delete polRecord['78k'];
      }
      if (polRecord['78K']) {
        delete polRecord['78K'];
      }

      definirPoliciais(polRecord);
    }

    // 3. Sync avisos
    if (Array.isArray(data.avisos) && (data.avisos.length > 0 || forcarPull)) {
      ...
      definirAvisos(mapped);
    }

    // 4. Sync testes
    if (Array.isArray(data.testes) && (data.testes.length > 0 || forcarPull)) {
      ...
      definirTestes(mapped);
    }

    // 5. Sync movimentacoes
    if (Array.isArray(data.movimentacoes) && (data.movimentacoes.length > 0 || forcarPull)) {
      ...
      definirMovimentacoes(mapped);
    }

    // 6. Sync categorias (SETORES!)
    if (Array.isArray(data.categorias) && (data.categorias.length > 0 || forcarPull)) {
      ...
      definirCategorias(mapped);
    }

    // 7. Sync patentes
    if (Array.isArray(data.patentes) && (data.patentes.length > 0 || forcarPull)) {
      ...
      definirPatentes(mapped);
    }

    // 8. Sync grupos
    if (Array.isArray(data.grupos) && (data.grupos.length > 0 || forcarPull)) {
      const mapped = data.grupos.map((row: any) => ({
        ...
        cargos: Array.isArray(row.cargos_grupo) ? row.cargos_grupo : (Array.isArray(row.cargos) ? row.cargos : [])
      }));
      definirGrupos(mapped);
    }

    // 9. Sync treinamentos
    if (Array.isArray(data.treinamentos) && (data.treinamentos.length > 0 || forcarPull)) {
      const mapped = data.treinamentos.map((row: any) => ({
        ...
        cargosQuePodemVer: Array.isArray(row.cargos_que_podem_ver) ? row.cargos_que_podem_ver : ['todos'],
      ...
      definirTreinamentos(mapped);
    }

    // 10. Sync aplicacoes_treinamentos
    if (Array.isArray(data.aplicacoes_treinamentos) && (data.aplicacoes_treinamentos.length > 0 || forcarPull)) {
      ...
      definirAplicacoesTreinamentos(mapped);
    }

    // 11. Sync xp_enviados
    if (Array.isArray(data.xp_enviados) && (data.xp_enviados.length > 0 || forcarPull)) {
      ...
      definirXpEnviados(mapped);
    }

    // 12. Sync advertencias
    if (Array.isArray(data.advertencias) && (data.advertencias.length > 0 || forcarPull)) {
      ...
      definirAdvertencias(mapped);
    }

    // 13. Sync exoneracoes
    if (Array.isArray(data.exoneracoes) && (data.exoneracoes.length > 0 || forcarPull)) {
      ...
      definirExoneracoes(mapped);
    }

    // 14. Sync solicitacoes_cadastro
    if (Array.isArray(data.solicitacoes_cadastro) && (data.solicitacoes_cadastro.length > 0 || forcarPull)) {
      ...
      definirSolicitacoesCadastro(mapped);
    }

    // 15. Sync posts_abas_customizadas
    if (Array.isArray(data.posts_abas_customizadas) && (data.posts_abas_customizadas.length > 0 || forcarPull)) {
      ...
      definirPostsAbasCustomizadas(mapped);
    }

    // 16. Sync apostilas
    if (Array.isArray(data.apostilas) && (data.apostilas.length > 0 || forcarPull)) {
  ...
  try {
    ...
    if (config) {
      payload.configuracao_site = [{
        ...
        grupos_permissoes: config.gruposPermissoes || {},
    ...
    if (avisos) {
      payload.avisos = avisos.map(a => ({
        ...
        negrito: !!a.negrito,
        italico: !!a.italico,
        marcar_everyone: !!(a as any).marcarEveryone,
    ...
    if (grupos) {
      payload.grupos = grupos.map(g => ({
        ...
        membros: g.membros,
        cargos_grupo: g.cargos || []
      }));
    }

    // 9. Treinamentos
    const treins = obterTreinamentos();
    if (treins) {
    ...
    const apls = obterAplicacoesTreinamentos();
    if (apls) {
    ...
    const sols = obterSolicitacoesCadastro();
    if (sols) {
