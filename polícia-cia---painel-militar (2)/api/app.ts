...
async function safeUpsert(tableName: string, payload: any): Promise<{ data: any; error: any }> {
  ...
  while (attempts < maxAttempts) {
    ...
    if (error.code === '23503' || errorMsg.toLowerCase().includes('foreign key')) {
      console.log(`[Safe Upsert] Foreign key constraint violation on table "${tableName}". Ensuring referenced parent records...`);
        ...
        if (item && typeof item === 'object') {
          ...
          if (item.treinamento_id) {
            try {
              ...
              console.warn('[Safe Upsert] Auto-creation of parent treinamento failed:', e);
            }
          }
        }
      }
  ...
  return { data: null, error: { message: `Não foi possível salvar na tabela "${tableName}". Verifique se a estrutura no Supabase está atualizada com o script SQL das Configurações.` } };
}
...
async function garantirAdminsIniciais() {
  ...
  // Deletar a conta antiga 'admin' se existir no Supabase, mantendo apenas PoliceCIA
  try {
    await client.from('policiais').delete().eq('nick', 'admin');
  } catch (err) {
    console.warn('[Server Startup] Não foi possível apagar admin antigo:', err);
  }
  ...
  await safeUpsert('policiais', [{
    ...
    avatar_habbo: 'PoliceCIA',
    pontos_promocao: 0,
    presencas: 999,
    streak: 99,
    senha_hex: criptografarTexto('admin#2026'),
