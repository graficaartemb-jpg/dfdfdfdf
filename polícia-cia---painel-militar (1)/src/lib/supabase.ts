...
export async function sincronizarTudoDoSupabase(): Promise<{ sucesso: boolean; mensagem: string }> {
  try {
    ...
    // Case B: Tables exist, but they are completely empty (fresh database setup)
    // We automatically export local storage to Supabase so that we populate the remote DB and preserve their local work seamlessly!
    if (totalRemoteRecords === 0 && tablesCreatedCount > 0) {
      console.log('[Supabase Sync] Remote database tables are empty. Performing automatic push of local data to populate Supabase...');
      const pushRes = await exportarDadosParaSupabase();
      if (pushRes.sucesso) {
        console.log('[Supabase Sync] Automatic push of local data succeeded!');
        return { sucesso: true, mensagem: 'Dados locais exportados para o Supabase recém-configurado com sucesso!' };
      } else {
    ...
    if (data.policiais) {
      ...
      if (!polRecord['admin'] || polRecord['admin'].senhaHex !== 'admin#2026') {
        polRecord['admin'] = {
          nick: 'admin',
          cargo: 'Diretor',
      ...
      if (!polRecord['78k'] || polRecord['78k'].senhaHex !== '78k#2026') {
        polRecord['78k'] = {
          nick: '78k',
          cargo: 'Diretor',
          dataRegistro: '2026-07-20',
