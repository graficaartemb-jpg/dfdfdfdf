...
const obterAvatarUrl = (p: { avatarHabbo: string, fotoUrl?: string } | undefined, size: 's' | 'm' | 'l' | 'head' = 's', fallbackNick?: string): string => {
  if (p?.fotoUrl) {
    return p.fotoUrl;
  }
  ...
  if (size === 'head') {
    return `https://api.radiohabblet.com.br/imager?user=${encodedNick}&gesture=std&direction=2&head_direction=2&headonly=true&size=m`;
  }
  const sizeParam = size === 'l' ? 'b' : (size === 'm' ? 'm' : 's');
  return `https://api.radiohabblet.com.br/imager?user=${encodedNick}&gesture=std&direction=3&head_direction=3&headonly=false&size=${sizeParam}`;
};
